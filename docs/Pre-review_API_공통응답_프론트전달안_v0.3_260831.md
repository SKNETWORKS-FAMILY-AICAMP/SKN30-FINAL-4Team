...
작성일: 2026-08-31 · 문서 버전: v0.3 · 서비스 API 12개 + 운영 API 2개
...
| 분석 상태 표시값 | 3개. 아직 시작하지 않은 건도 ‘분석 중’에 포함 | 4개. ‘분석 전’을 분리 |

마지막 행의 **분석 상태 4분류는 응답 바깥 구조와 별개인 계약 변경**입니다. 범위는 [8. 계약 변경](#8-계약-변경--분석-상태-4분류)에 따로 적었습니다.
...
current_password는 1~128자, new_password는 12~128자입니다. 현재 비밀번호와 새 비밀번호는 달라야 합니다. 변경 성공 후 기존 토큰은 서버가 거부합니다. 저장된 토큰을 제거하고 다시 로그인해야 합니다. 별도의 토큰 발급은 이 응답에 포함하지 않습니다.
...
본문·쿼리 파라미터 없음. 본인 분석 건을 최신순으로 최대 50건 조회합니다. title은 원본 파일명이며 없으면 null입니다. 이력이 없으면 data는 null이 아니라 {"cases": []}입니다. status는 8번과 같은 4개 값입니다.
...
case_id는 정수 경로 파라미터입니다. 본문 없음. 202는 접수 성공이며 분석 완료가 아닙니다. 반환되는 data.status는 PARSING입니다. 성공 후 8번 상태 조회를 시작합니다. job_id는 불투명 문자열이며 별도 job 조회 API는 없습니다. UPLOADED 상태, 곧 상태 조회의 '분석 전'에서만 분석을 시작할 수 있습니다. 이미 실패한 건은 같은 건으로 재분석할 수 없고 409입니다. 다시 업로드해 새 분석 건을 만들어야 합니다.
...
case_id는 정수입니다. 본문 없음. status는 ‘분석 전’, ‘분석 중’, ‘분석 완료’, ‘분석 실패’ 중 하나입니다. 분석 실패를 정상 조회해도 HTTP는 200 / code는 SUCCESS입니다. failure_code·failure_message는 분석 실패일 때 확인하며 API 오류 code·message와 구분합니다.

서버 내부 처리 단계와 표시 상태의 대응은 다음으로 고정합니다.

| 표시 status | 서버 내부 상태 | 뜻 |
|---|---|---|
| 분석 전 | UPLOADED | 업로드만 됨. 아직 분석을 시작하지 않음 |
| 분석 중 | PARSING, CHECKING, RETRIEVING, REPORTING | 분석 진행 중 |
| 분석 완료 | COMPLETED | 결과 조회 가능 |
| 분석 실패 | FAILED | 같은 건으로 재분석 불가 |

**내부 단계는 늘어날 수 있고 프론트에 그대로 노출하지 않습니다.** 늘어난 진행 단계는 ‘분석 중’에 흡수하며, 프론트는 위 4개 값만 처리합니다. 내부 상태 이름으로 분기하지 않습니다.
‘분석 전’은 폴링 대상이 아닙니다. 새로고침 뒤에도 상태 조회 한 번으로 다음 행동이 정해집니다.

> **적용 전 주의:** 현재 서버는 표시 상태가 3개이고 ‘분석 전’에 해당하는 건도 ‘분석 중’으로 내려옵니다. 4분류가 배포되기 전에는 상태값만으로 미시작과 진행 중을 구분할 수 없습니다. 8절을 확인하세요.
...
status가 ‘분석 실패’이면 failure_message를 표시하고 새 문서 업로드를 안내합니다. 같은 건에 대한 재분석 API는 없으므로 재시도 버튼은 7번이 아니라 5번 업로드로 연결합니다.
...
API 명세 파일의 9번 항목에도 같은 예시가 있지만, 그쪽은 **같은 모양의 반복을 줄인 축약본**입니다. self_check.items 13개 중 2개, relations 7개 중 2개만 실려 있습니다. 생략 없는 전체 응답이 필요하면 위 전체예시 파일을 봅니다.
...
4. 분석 상태 조회 → status 하나로 다음 행동을 정함
   - `data.status = "분석 전"`: 폴링하지 않고 분석 시작 버튼 표시
   - `data.status = "분석 중"`: 상태 조회 계속
   - `data.status = "분석 완료"`: 상태 조회 종료 후 보고서 조회
   - `data.status = "분석 실패"`: 상태 조회 종료 후 failure_message 표시와 새 문서 업로드 안내
5. 분석 결과로 화면 표시
6. report_download_url이 있으면 인증 헤더를 붙여 PDF 다운로드
7. 보고서가 준비된 분석 건에서 질문 전송·대화 이력 조회

폴링 간격·최대 대기시간은 여기서 임의로 고정하지 않습니다.
화면 진입·새로고침 뒤에는 상태 조회 결과만으로 4갈래를 정합니다. 시작 여부를 알아내려고 7번을 다시 호출하지 않습니다.
7번은 확인용 호출이 아닙니다. 202면 실제로 분석이 시작되고, 409는 진행 중·완료·실패를 모두 덮어 진행 중이라고 단정할 수 없습니다.
...
분석 이력·상태 조회는 한글 상태값 4개를 사용합니다.
업로드 응답은 UPLOADED, 분석 시작 응답은 PARSING, 보고서 ui_status는 COMPLETED로 영문 값을 그대로 씁니다.
이 문서는 응답 바깥 구조를 통일합니다. 표시 상태 4분류 외에 DB 상태값을 새로 만들거나 재설계하지 않습니다.
...
| Pre-review_API_명세_v0.3_260831.json | **JSON 명세 본체.** 1~14번 API를 순서대로 담은 배열 |
...
API별 명세는 **JSON 파일이 본체**이고 이 문서는 공통 규칙·호출 순서·표시 규칙을 담습니다.

[JSON 명세](Pre-review_API_명세_v0.3_260831.json)는 1번부터 14번까지를 순서대로 담은 배열이며, 항목 하나가 API 하나입니다. 항목마다 같은 10개 키를 가집니다.

| 키 | 내용 |
|---|---|
| `id` · `title` · `method` · `path` · `authentication_required` | 어떤 API인지 |
| `request` | content_type · path_parameters · query_parameters · body · form_fields · constraints |
| `success` · `additional_success_examples` | 성공 응답 형식과 추가 예시 |
| `errors` | 실패 응답. 항목마다 http_status · when · body |
| `notes` | 그 API에만 해당하는 계약과 주의사항 |

공통 응답 5필드·오류 코드표·처리 3갈래·분석 상태값은 이 문서 1·2·6절에 있고 JSON에 중복하지 않았습니다.

이 JSON은 **파일 전체가 서버 응답이 아닙니다.**
각 항목의 `success.body`, `errors[].body`가 실제로 제안하는 응답 본문입니다.
`request`는 모든 항목이 같은 6개 키(`content_type`·`path_parameters`·`query_parameters`·`body`·`form_fields`·`constraints`)를 가지며, 해당 없는 값은 null 또는 빈 배열입니다.
`notes`에는 그 API에만 해당하는 계약과 주의사항이 들어갑니다.
...
- 분석 상태 4분류는 공통 응답 적용과 별개 항목입니다. 8절 범위를 따로 확인합니다.

## 8. 계약 변경 — 분석 상태 4분류

공통 응답 구조 통일과 **별개인 변경**이므로 합의와 적용을 따로 관리합니다.

**바꾸는 것**

현재 표시 상태 3개에서는 업로드만 된 건과 분석이 도는 건이 모두 ‘분석 중’입니다. 프론트는 이 값만으로 분석 시작 버튼을 띄울지 폴링을 돌릴지 정할 수 없습니다. ‘분석 전’을 분리해 상태 조회 한 번으로 다음 행동이 정해지게 합니다.

**바꾸지 않는 것**

- DB 상태값은 그대로입니다. UPLOADED가 이미 있으므로 스키마 변경이 필요 없습니다.
- **실패 건의 재분석은 이 변경에 포함되지 않습니다.** 상태를 4개로 나눠도 FAILED 건은 여전히 409이고, 재분석을 허용하려면 별도 합의가 필요합니다.
- 내부 진행 단계(PARSING·CHECKING·RETRIEVING·REPORTING)를 프론트에 노출하지 않습니다.

**함께 바뀌는 곳**

코드 몇 줄 수정이 아니라 응답 계약 변경입니다. 최소한 다음이 함께 움직입니다.

| 대상 | 내용 |
|---|---|
| 표시 상태 매핑 | 내부 상태 → 표시 상태 변환에서 UPLOADED를 분리 |
| 응답·서비스 타입 | 상태 조회 응답, 이력 조회 응답, 서비스 반환 타입의 상태 값 집합을 4개로 확장 |
| 계약 테스트 | 이력 상태 매핑 테스트가 UPLOADED를 ‘분석 중’으로 고정하고 있어 기대값과 테스트 이름을 함께 변경 |
| OpenAPI | 응답 모델에서 자동 생성되므로 상태 enum이 4개로 갱신되는지 확인 |
| 프론트 | 4갈래 분기와 ‘분석 전’ 화면 처리 추가 |

**적용 순서**

프론트가 먼저 4갈래로 바뀌면 서버가 ‘분석 전’을 보내기 전까지 그 분기는 죽어 있고, 서버가 먼저 바뀌면 기존 프론트가 모르는 값을 받습니다. 어느 쪽이든 **모르는 상태값을 만나면 폴링을 멈추고 상태를 그대로 표시**하도록 프론트가 먼저 대비한 뒤 서버를 적용합니다.
