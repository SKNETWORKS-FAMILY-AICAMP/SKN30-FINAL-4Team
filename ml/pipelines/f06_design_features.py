...
OUT_V3 = os.path.join(C.PROC, "design_features_v3.parquet")
...
# --------------------------------------------------- 공급 보강 (M66, v3 전용)
#
# M64 §3 이 남긴 숙제 하나뿐이다: **얇은 비교군의 해법은 모델이 아니라 표본**
# **확대이고, 그 표본의 절반은 새 수집이 아니라 이미 가진 문서의 추출 개선이다.**
# 여기서 그 추출을 한다. 새 문서를 받지 않는다 — E01 이 이미 뽑아 둔, 그리고
# M65 가 이미 근거문으로 붙여 둔 그 텍스트만 다시 읽는다.
#
# 세 가지를 채운다. 순서가 곧 의존관계다.
#
#   S1 support_unit    '20개사 내외' 처럼 **지원 대상의 개수 표현 자체**가 단위를
#                      말하는 자리를 근거등급 한 단으로 추가한다.
#   S2 amount_type     S1 이 만든 **문서 근거 단위**로 `unknown` 을 해소한다.
#                      taxonomy 에서 resolve_amount_type 이 코호트 관행으로 하던
#                      일을, bizinfo 에서는 문서 근거로 한다.
#   S3 support_ratio   금액 문맥·지원규모 절에서 '무엇의 몇 %' 를 읽는다.
#
# **채우기만 한다.** 이미 값이 있는 칸은 건드리지 않는다 — 멀쩡한 값을 새 규칙으로
# 갈아엎으면 '데이터를 보강했다'가 아니라 '규칙을 바꿨다'가 되고, 그 순간 M64 와의
# 비교가 성립하지 않는다(M65 가 taxonomy 지원단위에서 지킨 것과 같은 규율).
#
# 그리고 **문서가 있는 행 전체에 균일하게** 적용한다. 얇은 비교군에만 적용하면
# 늘리고 싶은 칸만 늘어나 '보강했더니 그 칸이 커졌다'가 동어반복이 된다.

# S1 -----------------------------------------------------------------------
# COUNT_RE 가 잡는 단위 토큰 -> support_unit. 어휘를 새로 만들지 않는다 —
# F05 가 support_count 를 뽑을 때 쓴 바로 그 정규식의 매치를 다시 읽을 뿐이라
# 뽑힌 개수와 그 개수의 단위가 **정의상 어긋날 수 없다**.
# 맨 `개` 는 무엇의 개수인지 알 수 없어 단위로 쓰지 않는다.
COUNT_TOKEN_UNIT = [(re.compile(r"개사내외|개사|개\s*기업"), "company"),
                    (re.compile(r"개\s*과제"), "project"),
                    (re.compile(r"개\s*팀"), "team")]


def unit_from_count(text):
    """support_count 를 뽑은 그 표현이 무엇을 세었는지 읽는다."""
    if not isinstance(text, str) or not text.strip():
        return None, None, ""
    for m in AP.COUNT_RE.finditer(text):
        for rx, u in COUNT_TOKEN_UNIT:
            if rx.search(m.group(0)):
                return u, m.group(0), text[max(0, m.start() - 30):
                                           m.end() + 30].replace("\n", " ")
    return None, None, ""


# S2 -----------------------------------------------------------------------
# 문서에서 읽은 단위만 amount_type 해소에 쓴다. `amount_type_convention` 은
# amount_type 에서 되돌린 값이라 그것으로 amount_type 을 정하면 순환이다.
DOC_UNIT_BASIS = {"amount_context", "scope_section", "count_expression"}
UNIT_TO_TYPE = {"company": "per_company", "project": "per_project",
                "team": "per_project"}

# S3 -----------------------------------------------------------------------
# 부담 쪽 어휘 — 이것이 % 를 지배하면 지원비율은 (100 - N) 이다
_BURDEN = (r"자\s*부담|자기\s*부담|기업\s*부담|민간\s*부담|기업\s*분담|민간\s*분담|"
           r"기관\s*부담|주관기관\s*부담|참여기업\s*부담|선정기업\s*부담|"
           r"대응\s*투자|부담금|분담금")
# 지원 쪽 어휘 — 이것이 % 를 지배하면 지원비율은 N 이다
_GRANT = (r"정부\s*지원금|지원금|지원\s*금액|국비|도비|시비|보조금|출연금|"
          r"지원\s*한도|지원\s*비율|보조율|지원율")
_COST = (r"사업비|총\s*사업비|소요\s*비용|소요\s*액|비용|경비|참가비|참여비|제작비|"
         r"보험료|보증료|임차료|요금|운임|공급가액|단가|등록료|심사비|인증비|"
         r"시험비|훈련비|교육비|컨설팅\s*비|연구개발비")
PCT_RE = re.compile(r"(\d{1,3}(?:\.\d)?)\s*%")
BURDEN_RE = re.compile(_BURDEN)
# '무엇의 N%' — 비용어/지원어가 조사 '의' 로 % 를 **직접** 지배할 때만 인정한다.
# 문장 어디에 비용어가 있기만 하면 잡는 식으로 하면 문서 길이가 비율을 정한다.
COST_GOV_RE = re.compile(r"(?:" + _COST + r")\s*(?:등)?\s*(?:\([^)]{0,25}\))?\s*"
                         r"의\s*(?:최대\s*|최고\s*)?$")
GRANT_GOV_RE = re.compile(r"(?:" + _GRANT + r")\s*(?:등)?\s*[(:：]?\s*(?:최대\s*)?$")
# % 매치 **바로 뒤**에 붙는 부담 표현. 왼쪽 창처럼 느슨하게 보면
# '사업비의 80% 지원 (기업부담 20%)' 에서 80 을 20 으로 뒤집는다.
RIGHT_BURDEN_RE = re.compile(r"^\s*(?:이상|이하|내외|정도)?[을를]?\s*(?:현금|현물)?\s*"
                             r"(?:매칭\s*)?(?:" + _BURDEN + r")")
# % 이지만 지원비율이 아닌 것
RATIO_DISQ_RE = re.compile(r"금리|이자율|요율|율\s*우대|증가율|감소율|성장률|"
                           r"고용보험|산재보험|보증금|기술료|로열티|환수|반환|"
                           r"판매\s*수수료|중개\s*수수료|이상인|미만인|이하인|"
                           r"비율이|지분|점유율|달성률")
RATIO_LEFT, RATIO_RIGHT = 40, 16


def _ratio_in(frag, tier):
    """한 조각에서 지원비율 후보 하나. 방향(지원/부담)을 정하는 것은 **지배어**다."""
    if not frag:
        return None
    for m in PCT_RE.finditer(frag):
        v = float(m.group(1))
        if not (0 < v <= 100):
            continue
        left = frag[max(0, m.start() - RATIO_LEFT):m.start()]
        right = frag[m.end():m.end() + RATIO_RIGHT]
        if RATIO_DISQ_RE.search(left) or RATIO_DISQ_RE.search(right):
            continue
        win = (left + m.group(0) + right).replace("\n", " ")
        gov = COST_GOV_RE.search(left) or GRANT_GOV_RE.search(left)
        tight = RIGHT_BURDEN_RE.search(right)
        # 지배어가 바로 왼쪽에 붙어 있으면 그것이 방향을 정한다. 단 '…의 25%
        # 이상 민간부담' 처럼 바로 뒤가 부담이면 뒤집는다.
        if gov:
            return ((round(100.0 - v, 2), tier + ":gov_burden") if tight
                    else (v, tier + ":share")) + (m.group(0), win)
        if tight or BURDEN_RE.search(left):
            return round(100.0 - v, 2), tier + ":burden", m.group(0), win
    return None


def derive_ratio_graded(text):
    """지원비율·근거등급·매치문자열·근거창. 창은 지원단위와 같은 두 단이다."""
    for frag, tier in ((amount_context(text), "amount_context"),
                       (scope_text(text), "scope_section")):
        r = _ratio_in(frag, tier)
        if r:
            return r
    return None, "none", None, ""
...
def apply_supply(df):
    """S1~S3 을 채우고, 무엇이 몇 건 채워졌는지 원장을 함께 돌려준다."""
    d = df.copy()
    for c in ("support_ratio_basis", "amount_type_basis"):
        d[c] = None
    txt = d["evidence_text"]

    # S1 지원단위 — 근거가 없어 비어 있던 칸만
    n1 = 0
    for i in d.index[d["support_unit"].isna()]:
        u, hit, win = unit_from_count(txt.at[i])
        if u:
            d.at[i, "support_unit"] = u
            d.at[i, "support_unit_basis"] = "count_expression"
            d.at[i, "support_unit_hit"] = hit
            d.at[i, "support_unit_window"] = win
            n1 += 1

    # S2 금액 의미 — 문서 근거 단위가 있는 unknown 만
    m2 = (d["amount_type"] == "unknown") & d["support_unit"].isin(UNIT_TO_TYPE) \
        & d["support_unit_basis"].isin(DOC_UNIT_BASIS)
    d.loc[m2, "amount_type"] = d.loc[m2, "support_unit"].map(UNIT_TO_TYPE)
    d.loc[m2, "amount_type_source"] = "doc_unit_evidence"
    d.loc[m2, "amount_type_basis"] = d.loc[m2, "support_unit_basis"]
    n2 = int(m2.sum())

    # S2 를 반영해 기업당 지원액을 다시 계산한다 (채우기만 — 기존 값 유지)
    pr = [per_recipient(a, mx, c) for a, mx, c in
          zip(d["amount_type"], d["amount_max"], d["support_count"])]
    newpr = pd.Series([v for v, _ in pr], index=d.index)
    newbs = pd.Series([b for _, b in pr], index=d.index)
    fill = d["per_recipient"].isna() & newpr.notna()
    d.loc[fill, "per_recipient"] = newpr[fill]
    d.loc[fill, "per_recipient_basis"] = newbs[fill]
    n2b = int(fill.sum())

    # S3 지원비율
    n3 = 0
    for i in d.index[d["support_ratio"].isna()]:
        v, basis, hit, win = derive_ratio_graded(txt.at[i])
        if v is not None:
            d.at[i, "support_ratio"] = v
            d.at[i, "support_ratio_basis"] = basis
            n3 += 1

    ledger = {"S1_지원단위_count_expression": n1,
              "S2_금액의미_문서근거해소": n2,
              "S2_기업당지원액_신규": n2b,
              "S3_지원비율_신규": n3}
    return d, ledger
...
PROVENANCE_COLS = ["evidence_source", "support_unit_basis", "support_unit_hit",
                   "support_unit_window"]
SUPPLY_COLS = ["support_ratio_basis", "amount_type_basis"]
...
def main(fixed=True, out=None, supply=False):
    out_override = out is not None
    out = out or (OUT_V3 if supply else (OUT_V2 if fixed else OUT))
    df = pd.concat([build_taxonomy(fixed), build_bizinfo(fixed)], ignore_index=True)

    supply_ledger = {}
    if supply:
        if not fixed:
            raise ValueError("공급 보강은 근거문 수정본 위에서만 의미가 있다 (--legacy 와 함께 못 쓴다)")
        df, supply_ledger = apply_supply(df)
    ...
    print("[data] %s  %s  (%s)"
          % (out, df.shape, "공급 보강본" if supply
             else ("근거문 수정본" if fixed else "legacy 재현")))
    if supply_ledger:
        print("  공급 보강:", supply_ledger)
    if sanity:
    ...
    if out_override:
        ...
        return df
    C.save_report("f06_design_features%s.json"
                  % ("_v3" if supply else ("_v2" if fixed else "")), {
        "output": os.path.relpath(out, C.ROOT),
        "evidence_fix": bool(fixed),
        "supply_fill": supply_ledger or None,
        "rows": int(len(df)),
...
if __name__ == "__main__":
    ...
    ap.add_argument("--supply", action="store_true",
                    help="이미 가진 문서에서 지원단위·금액의미·지원비율을 더 뽑아 "
                         "design_features_v3.parquet 을 만든다 (M66)")
    a = ap.parse_args()
    main(fixed=not a.legacy, out=a.out, supply=a.supply)
