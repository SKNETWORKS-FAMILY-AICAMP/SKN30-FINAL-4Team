...
from common import (ATT, BIZINFO, DETAIL_URL, PROC, REPORTS, report_path, UA, safe_name,
                    save_report)

OUT_DIR = os.path.join(ATT, "list")
MANIFEST = report_path("d04_manifest_list.csv")
...
def pick_file(html, prefer_hwp=False):
    """상세페이지에서 받을 문서 1개를 고른다. (section, name, ext, url) 또는 None.

    prefer_hwp 를 켜면 같은 후보군 안에서 HWP/HWPX 를 먼저 고른다. F05 가
    PDF 를 EXCLUDE_EXT 로 버리기 때문에(표 셀 병합으로 금액이 왜곡된다),
    HWP 가 있는데 PDF 를 받으면 그 공고는 관측 단계에서 통째로 사라진다.
    기본값은 꺼짐 — D02 5,000건 표본의 기존 수집 결과를 그대로 재현한다."""
    ...
    pool = body if body else [c for c in cands if c["ext"] in DOC_EXT]
    if not pool:
        return None
    if prefer_hwp:
        hw = [c for c in pool if c["ext"] in ("hwp", "hwpx")]
        if hw:
            return hw[0]
    return pool[0]
...
def main():
    ...
    ap.add_argument("--prefer-hwp", action="store_true",
                    help="같은 공고에 HWP/HWPX 가 있으면 PDF 대신 그것을 받는다. "
                         "F05 가 PDF 를 버리므로 신규 수집에서는 켜는 편이 수율이 높다.")
    ...
    if args.manifest:
        ...
        manifest_path = report_path("d04_manifest_%s.csv" % stem)
    ...
    for n, pid in enumerate(todo, 1):
        ...
        try:
            ...
            if page.status_code != 200:
                ...
                cand = pick_file(page.text, prefer_hwp=args.prefer_hwp)
                if cand is None:
                    rec["status"] = "no_doc"
                    nodoc += 1
                else:
