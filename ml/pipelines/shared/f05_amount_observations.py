...
이 테이블은 파이프라인의 뿌리다. 아래가 전부 이걸 읽는다.
    f06_design_features   모델 2·3·4 공통 feature
    a01_eda_plots               전처리 검증 EDA
    m09_coverage_accuracy    커버리지-정확도 트레이드오프
...
근거 수치는 `ml/reports/a02_ts_stl_diagnosis.json` 과 `a03_ts_trend_test.md` 에 남아
...
from common import PROC, REPORTS, report_path, save_report, mark_outliers, SANE_RANGE
...
# m08 은 50.6% 만 신뢰로 매기고 그 중 78.3% 가 맞았다(M07). 과신 의심이 있어
...
# M08 가 목록 표본(2019~2025)에 붙인 지원성격. machine-learning 단계 산출물이라
# 이 브랜치에 반드시 있다(상류 -> 하류 방향).
LIST_CLASSIFIED = PROC + "/list_sample_support_type.parquet"
SAMPLE = PROC + "/list_sample.parquet"
# E01(pdf-inspector + rhwp)가 있으면 우선 사용한다. HWP 표가 실제로 읽힌다.
# 단 PDF 는 EXCLUDE_EXT 로 걸러낸다(아래 근거 참조).
DOCS_V2 = report_path("e01_documents.jsonl")
DOCS_API = report_path("e01_documents_api.jsonl")
DOCS_LIST = report_path("e01_documents_list.jsonl")
...
def load_docs(path, source=None, exclude_ext=EXCLUDE_EXT):
    ...
    source 를 주면 해당 출처(api/list)만 추린다. E01 는 두 출처를 한 파일에 담는다.
...
def attach_support_type(d, source):
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(
            "지원성격 산출물이 없다: %s\n"
            "  --support-type-source m08 은 M02 을, dl05 는 DL05 를 먼저 실행해야 한다.\n"
...
def load_list_support_type():
    """M08 가 목록 표본에 붙인 지원성격 라벨. 판단보류는 라벨로 쓰지 않는다.
    ...
    if not os.path.exists(LIST_CLASSIFIED):
        ...
        print("       m08_apply_list_sample.py 를 먼저 실행하면 지원성격 축 커버리지가 크게 는다.")
        return {}
...
def build_observations(source):
    ...
    # 모델 1 이 모델 2 에 기여하지 못했다. M08 가 이 표본에 라벨을 붙인다.
    ...
    # 상식 범위 밖 = 파싱 오류. 행을 지우지 않고 플래그만 붙여 하류(s03f/s08a/s09a)가
...
def main():
    ...
    save_report("f05_amount_observations.json", {
