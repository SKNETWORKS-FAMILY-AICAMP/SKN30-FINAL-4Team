"""M45 — 모델 2: 유사 비교군 안에서의 기업당 지원규모 (이전 canonical).

M56 으로 canonical 이 넘어갔다(2026-08-27). 바뀐 것은 feature 와 학습기
둘뿐이고, 아래의 타깃 정제·비교군 사다리·percentile 조회·구간 실용성
등급은 M56 이 이 파일을 그대로 import 해서 쓴다. 이 파일은 지우지 않는다 —
여기 적힌 수치(MAE 0.4681 / 개선 11.9% / 커버리지 79.6% / 폭 33.2배)가
그 시점의 확정값이고, 승격 근거는 같은 조건에서 잰 비교표에 있다.
    새 canonical   ml/scripts/m56_m2_canonical.py
    feature 고정   ml/scripts/m2_features.py
    누수 감사      ml/reports/m55_m2_leakage_audit.md
...
def write_md(ref, top, res, tiers, demo, drop, verdict, main_tag):
    ...
    p = C.report_path("m45_m2_amount.md")
