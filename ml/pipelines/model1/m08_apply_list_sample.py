"""M08 — 지원성격 분류기를 목록 표본(2019~2025)에 적용.
...
왜 필요한가
    ...
    막혀 있던 건 커버리지였다. M02 은 Open API(2025~2026) 1,570건만 다루고,
    장기 시계열의 뼈대인 목록 표본(2019~2025)에는 라벨이 없었다. 이 스크립트가
    그 구멍을 메운다.

적용 조건 — M02 과 맞춘다
    학습        business_taxonomy 전량 (2022+2023, MIN_SUPPORT=10 → 19클래스)
    입력        e01_documents.jsonl 의 source="list" 원문 + clean_text 전처리
    임계값      0.25 / 0.35 (M02 과 동일)
    PDF 제외    표 셀이 뭉쳐 원문이 깨진다. F05 가 같은 이유로 이미 제외하고 있어,
                목록 표본 관측 1,720건은 전부 HWP 계열이다. 여기서도 같게 맞춘다.

    M02 은 조건 B(E01 원문 + 전처리)를 골랐지만, 목록 표본 쪽은 F05 가 E01 를
    쓰므로 여기서도 E01 로 맞춘다. 같은 문서를 두 스크립트가 다르게 읽으면
    관측과 라벨이 어긋난다.
...
from common import PROC, REPORTS, report_path, save_report
from m01_support_type import MIN_SUPPORT, coarsen, tfidf
from m02_apply import HOLD_THRESHOLD, TRUST_THRESHOLD, clean_text, tier

TAX = PROC + "/business_taxonomy.parquet"
MASTER = PROC + "/announcement_master.parquet"
DOCS = report_path("e01_documents.jsonl")
OUT = PROC + "/list_sample_support_type.parquet"

EXCLUDE_EXT = {"pdf"}          # F05 와 동일 기준
DOC_SOURCE = "list"


def load_list_docs(path, exclude_ext=EXCLUDE_EXT):
    """source=list 원문 중 가장 긴 것을 공고별로 하나 고른다(F05 와 같은 규칙)."""
    best = {}
    if not os.path.exists(path):
        raise FileNotFoundError(
            "원문 추출본이 없다: %s\n  e01_extract_text.py 를 먼저 실행해야 한다." % path)
...
def main():
    ...
    print("학습: %d건 / %d클래스 (M02 과 동일)" % (len(sub), len(classes)))
    ...
    save_report("m08_apply_list_sample.json", {
        ...
        "exclusion_reason": ("PDF 는 표 셀이 뭉쳐 원문이 깨진다. F05 가 같은 기준으로 "
