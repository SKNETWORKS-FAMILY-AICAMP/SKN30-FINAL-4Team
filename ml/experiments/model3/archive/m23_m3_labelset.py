...
import common as C
from m13_m3_anomaly import AXIS_LABEL, MIN_AXES, REF, SRC, explain, prepare
from m16_m3_ocsvm_tuning import EXPERIMENTS, encode

OUT_SHEET = C.report_path("m23_m3_labelset.csv")
OUT_KEY = C.report_path("m23_m3_labelset_key.csv")
SEED = 42
N_TOTAL = 50
...
def main():
    ...
    with open(C.report_path("m16_m3_ocsvm_tuning.json"), encoding="utf-8") as f:
        m16 = json.load(f)
    with open(C.report_path("m20_m3_threshold.json"), encoding="utf-8") as f:
        m20 = json.load(f)
    ...
    C.save_report("m23_m3_labelset.json", {
...
def write_md(sheet, pol, verdict):
    ...
    for r in verdict["reasons"]:
        L.append("- %s" % r)
    L.append("")
    p = C.report_path("m23_m3_labelset.md")
