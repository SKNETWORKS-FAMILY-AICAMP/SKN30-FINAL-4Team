"""DL10 — 모델 4 딥러닝: Autoencoder 복원오차 기반 이상탐지 + Ablation.

설계서의 모델 4 우선순위 4위가 Autoencoder 다.
1~3위(IsolationForest / LOF / One-Class SVM)는 M13 에서 이미 쟀다.

기준선 (M13, 같은 2,339행 · 같은 인코딩 · 같은 합성 이상치)
...
AE 를 쓰는 논리
    ...
    끌려간 것"(M13 축별 상관 0.53/0.50)이었으므로, AE 가 같은 함정에 빠지는지
    똑같은 방식으로 다시 잰다.

평가는 M13 와 동일하게 네 갈래 — 정답이 없는 비지도 모델이라 잣대를 바꾸면
비교 자체가 무의미해진다.
...
import common as C
from m13_m3_anomaly import (MIN_AXES, REF, SRC, encode, explain, inject_synthetic,
                            norm01, prepare, score_drivers, status_of)
from dl08_m2_cluster import AE
...
def eval_synthetic(train, cfg, n=60):
    """M13 와 같은 합성 이상치·같은 시드. 잣대를 바꾸면 비교가 무의미해진다."""
...
def main():
    ...
    print("기준선(M13): IsolationForest 0.083 / LOF 0.633 / OneClassSVM 0.783")
    ...
    print("\n== 점수를 끌고 가는 축 (M13 의 IF 실패 원인을 같은 방식으로 확인)")
    ...
    C.save_report("dl10_m3_autoencoder.json", {
