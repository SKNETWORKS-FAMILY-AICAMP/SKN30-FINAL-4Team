...
OUT_CSV = os.path.join(C.REPORTS, "m31_parser_audit.csv")
...
def source_texts():
    ...
    docs = {}
    for path in (os.path.join(C.REPORTS, "e01_documents.jsonl"),):
...
def write_md(a, vc, n_bad, full):
    ...
    L += ["",
          ...
          "```", ""]
    p = os.path.join(C.REPORTS, "m31_m3_parser_audit.md")
