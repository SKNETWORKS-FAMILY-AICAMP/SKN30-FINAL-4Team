...
산출
    ml/models/m65_model2_canonical/{model2_canonical.joblib,manifest.json}
    ml/data/processed/m65_oof_predictions.parquet
    ml/reports/m65_m2_canonical_v2.json / .md

`models/_archive/m56_model2_canonical/` 은 지우지 않는다 — M45 를 남긴 것과 같은 이유다.
"""
...
def write_md(r):
    ...
    if lg["differing_columns"]:
        ...
        for k, v in lg["differing_columns"].items():
            A("| `%s` | %d |" % (k, v))
        A("")
        A("근거문과 무관한 컬럼이 달라졌다는 것은 **상류 산출물이 v1 시점과**")
        A("**어긋나 있다**는 뜻입니다. 이 상태로 재적합하면 근거문 수정의 효과와")
        A("상류 어긋남이 섞이므로, 먼저 상류를 맞춘 뒤에 다시 재야 합니다.\n")
    else:
        A("**34개 컬럼 3,963행이 전부 일치합니다.** 이 검증이 중요한 이유가 있습니다 —")
        A("처음 돌렸을 때는 `support_count` 82행 · `extraction_confidence` 55행이")
        A("달랐습니다. 원인은 근거문이 아니라 **상류 산출물이 어긋나 있던 것**이었고")
        A("(`business_taxonomy.parquet` 의 오래된 사본이 M32 의 지원기업수 파싱")
        A("수정 이전 값을 담고 있었습니다 — `6개월` 을 6개사로, `110개소` 를 110개사로")
        A("읽던 값), `f03_taxonomy.py` 로 재생성해 맞춘 뒤에야 완전 일치가 나왔습니다.")
        A("**그 상태로 재적합했으면 근거문 수정의 효과와 상류 어긋남이 섞였을**")
        A("**것입니다.** legacy 재현을 승격 절차 앞에 둔 이유가 이것입니다.\n")
    dt = a["determinism"]
    ...
    A("")
    A("`models/_archive/m56_model2_canonical/` 은 지우지 않습니다 — M45 를 남긴 것과 같은 이유이고,")
    A("되돌릴 자리를 남겨 둡니다.\n")
    A("조회 예시(서빙 경로 그대로):\n")
    A("```text")
    A("%s" % c["demo"]["title"])
