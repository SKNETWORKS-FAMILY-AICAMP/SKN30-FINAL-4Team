"""DL09 — 모델 3 딥러닝: 지원규모 quantile 회귀 MLP + Ablation.

기준선 (M12, 같은 데이터·같은 분할)
...
import common as C
from m12_m3_cohort import AXES, MIN_IMPROVEMENT, prepare
...
def fit_predict(Xtr, ytr, Xte, cfg, seed=SEED):
    ...
    # 분포가 학습에 새어 든다(M05 에서 잡았던 누수와 같은 종류다).
...
def main():
    ...
    print("기준선(M12): 코호트중앙값 0.6790 / LGBM-quantile50 0.5160")
    ...
    C.save_report("dl09_m3_cohort.json", {
