"""DL03 — Early stopping vs 고정 epoch 비교 (Nested split).

DL02 ablation 에서 epochs=12 가 최적임을 찾았다(Macro F1 0.7194).
...
비교 대상 (같은 outer fold, 같은 seed)
    A. 고정 epoch 12          — DL02 최적값, inner split 없이 train 720 전체 사용
...
BASELINE_DL12 = 0.7194    # DL02 최적 (epochs=12, train 720 전체)
...
def main():
    ...
    out = os.path.join(OUTDIR, "dl03_early_stopping_%s.json" % args.tag)
