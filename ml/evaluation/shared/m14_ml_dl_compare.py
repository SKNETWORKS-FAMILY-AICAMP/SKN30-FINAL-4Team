"""M14 — 모델 2·3·4 의 ML / DL 성능 통합 비교.

M11~M13(ML)와 DL08~DL10(DL)이 낸 측정값을 한 표로 모은다. 새로 학습하지 않고
...
def main():
    m2ml, m2dl = load("m11_m2_cluster.json"), load("dl08_m2_cluster.json")
    m3ml, m3dl = load("m12_m3_cohort.json"), load("dl09_m3_cohort.json")
    m4ml, m4dl = load("m13_m3_anomaly.json"), load("dl10_m3_autoencoder.json")
    ...
    p = os.path.join(R, "m14_ml_dl_compare.md")
    with open(p, "w", encoding="utf-8") as f:
        f.write("\n".join(L))
    print("\n".join(L))
    print("\n[report] %s" % p)

    C.save_report("m14_ml_dl_compare.json", {
        "model2": {"ml": "m11_m2_cluster.json", "dl": "dl08_m2_cluster.json", "verdict": m2v},
        "model3": {"ml": "m12_m3_cohort.json", "dl": "dl09_m3_cohort.json", "verdict": m3v},
        "model4": {"ml": "m13_m3_anomaly.json", "dl": "dl10_m3_autoencoder.json", "verdict": m4v},
    })
