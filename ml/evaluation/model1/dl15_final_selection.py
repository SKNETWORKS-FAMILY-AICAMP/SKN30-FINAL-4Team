...
읽는 것
    ...
    모델 3  m30_m3_real_eval.json        사람 라벨 50건에서 OneClassSVM
            ...
            m20_m3_threshold.json        합성 이상치 기준(병기용)
...
BUNDLE = os.path.join(C.MODELS, "model1_canonical", "external.parquet")
...
def main():
    ...
    m20 = load("m20_m3_threshold.json")
    ...
    report = {
        ...
        "model3": {"ocsvm_real": (m30 or {}).get("results"),
                   ...
                   "synthetic_reference": "m20_m3_threshold.json"},
...
def write_md(r, m1_rows, m1_meta, m29, m27, dl14, m26, m30, dl13, m20,
    ...
    with open(C.report_path("dl15_final_selection.md"), "w",
              encoding="utf-8") as f:
        f.write("\n".join(L))
