...
그래서 이 스크립트는 아무것도 학습하지 않는다. 이미 저장된 산출물만 읽는다.
    ...
    models/model1_canonical/*.parquet 학습 1,404건 / 외부 131건(라벨 확신도)
...
def main():
    with open(C.report_path("dl12_m1_candidates_dl.json"), encoding="utf-8") as f:
        dl12 = json.load(f)
    with open(C.report_path("dl12_m1_candidates_ml.json"), encoding="utf-8") as f:
        ml12 = json.load(f)
    ...
    rep = {
        "note": "학습 없음. dl12(DL/ML)·dl16·model1_canonical 재집계",
        "adopted": ADOPTED,
...
def write_md(r, blocks, train_counts):
    ...
    p = C.report_path("m57_m1_confidence_split.md")
