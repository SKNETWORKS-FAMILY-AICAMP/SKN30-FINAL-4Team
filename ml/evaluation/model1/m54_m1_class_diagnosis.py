...
그래서 이 스크립트는 **아무것도 학습하지 않는다.** 이미 저장된 산출물만 읽는다.
    ...
    models/model1_canonical/*.parquet 학습 1,404건 / 외부 131건(라벨 확신도 포함)
...
BUNDLE = os.path.join(C.MODELS, "model1_canonical")
ADOPTED = "KLUE-BERT"


def load():
    with open(C.report_path("dl12_m1_candidates_dl.json"), encoding="utf-8") as f:
        dl12 = json.load(f)
    with open(C.report_path("dl16_m1_abstention.json"), encoding="utf-8") as f:
        dl16 = json.load(f)
    with open(C.report_path("m03_input_alignment.json"), encoding="utf-8") as f:
        m03 = json.load(f)
...
def main():
    ...
    C.save_report("m54_m1_class_diagnosis.json", {
        "note": "학습 없음. 저장된 산출물(dl12/dl16/m03/model1_canonical)만 재집계",
        "adopted": ADOPTED,
...
def write_md(pc, weak, fl, conf_tbl, doc_tbl, cov_rows, m03, acc, gold, tr):
    ...
    p = C.report_path("m54_m1_class_diagnosis.md")
