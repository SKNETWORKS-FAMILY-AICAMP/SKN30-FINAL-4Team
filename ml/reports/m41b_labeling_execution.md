# M41b — 70건 라벨링 실행 기록 (freeze · blind · 2인 · finalize)
...
```text
n            70건 (중복 없음)
sha256       528d40ecedd538cc92ed183120f24bee25ebe15dbbb85506f3b091effe02525e
...
```text
대상   70건에서 무작위 20건 (1차 라벨을 보지 않고 시드로 선정)
A      claude-A (전체 70건)
B      claude-B (서브에이전트, 20건, A의 라벨 미열람)
```

| 지표 | 값 |
|---|---:|
| 단순 agreement | 18/20 = **90%** |
...
### 불일치 2건 — 전부 `data_error` 경계입니다

| row_id | A | B | 주 평가셋 영향 |
|---|---|---|---|
| `PBLN_000000000047357` | `data_error` | `uncertain` | 없음 |
| `PBLN_000000000077818` | `data_error` | `atypical_design` | **있음** |

두 건 모두 `기업당액_산출=budget_div_count` 인 행입니다. A 는 파생값이
...
```text
주 평가(Clean second hold-out)  normal + atypical_design = 53건 (양성 9건)
...
| 층 | `atypical_design` | `data_error` | `normal` | `uncertain` |
|---|---:|---:|---:|---:|
| `A_직접기재` | 9 | 3 | 40 | 0 |
| `B_파생` | 0 | 6 | 4 | 3 |
| `C_의심` | 0 | 5 | 0 | 0 |
...
데이터품질 층화는 정확히 먹혔습니다 — `C_의심` 5건이 **전부**
