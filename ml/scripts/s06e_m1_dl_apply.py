"""S06E — KLUE-RoBERTa 로 Open API 재적용 (S05B 의 딥러닝 버전).

M08은 LogisticRegression 으로 Open API 1,570건에 지원성격을 추론했고
판단보류가 70.5%(1,107건)였다. 그런데 S06D(MC Dropout)에서 학습 도메인
...
S05B 과의 비교가 성립하도록 조건을 동일하게 맞춘다.
    - 학습: 2023 엑셀 전량(900건, 26클래스)
    - 적용: Open API 1,570건, 원문 있으면 원문 / 없으면 요약문 대체
    - 전처리: S05B 조건 B(관인부 제거 + 본문 마커 발췌) — S05B 최적
    - 임계값: 0.25 / 0.35 (동일)

S06D 에서 얻은 epistemic 기준도 함께 산출한다. softmax 임계값은 오답을
...
DOCS = DATA_DIR + "/s02a_documents_api.jsonl"

MODEL = "klue/roberta-base"
MIN_SUPPORT = 3
CFG = {"lr": 3e-5, "epochs": 12, "batch": 16, "max_len": 256}  # S06B 최적
T_SAMPLES = 30
HOLD, TRUST = 0.25, 0.35

# S05B (LogisticRegression) 결과 — 비교 기준
S05B = {"model": "TF-IDF + LogisticRegression", "condition": "B (S02A + 전처리)",
...
def main():
    ...
    print("설정: %s" % CFG)
    print("비교 대상 — S05B(%s): 판단보류 %.1f%% (%d건)"
          % (S05B["model"], S05B["hold_rate"] * 100, S05B["n_hold"]))
    print(flush=True)
    ...
    print("%-34s%10d%10d%10d%9.1f%%"
          % ("S05B (LogisticRegression)", S05B["n_trust"], S05B["n_ref"],
             S05B["n_hold"], S05B["hold_rate"] * 100))
    print("%-34s%10d%10d%10d%9.1f%%"
          % ("S06E (KLUE-RoBERTa)", n_trust, n_ref, n_hold, n_hold / len(texts) * 100))
    print("=" * 74)
    print("평균 확신도: S05B %.4f → S06E %.4f (%+.4f)"
          % (S05B["mean_confidence"], conf.mean(), conf.mean() - S05B["mean_confidence"]))
    print("사용가능:    S05B %d건 → S06E %d건 (%+d)"
          % (S05B["n_trust"] + S05B["n_ref"], n_trust + n_ref,
             (n_trust + n_ref) - (S05B["n_trust"] + S05B["n_ref"])))
    ...
    rp = os.path.join(OUTDIR, "s06e_m1_dl_apply_%s.json" % args.tag)
