"""M09 — 학습/추론 입력 형식 정합이 판단보류율에 미치는 영향.
...
배경
    ...
    M08 에서 원문 품질(E01/E02)과 전처리 유무를 4조건으로 갈랐지만, 넷 다
    "학습과 형식이 다른 입력" 이라는 공통점을 공유했다. 그래서 원문을 더 많이,
    더 좋은 품질로 넣은 조건 D 가 오히려 보류율이 높았다(70.5% -> 73.8%).
    이 스크립트는 그 공통 전제 자체를 바꿔서 측정한다.

측정 설계
    모델·임계값·학습데이터를 M08 과 완전히 동일하게 두고 **추론 입력 구성만** 바꾼다.
    그래야 차이가 입력 형식에서 온 것임이 분리된다.

    E0. 현행 (title + summary + target + hashtags)      <- M08 fallback 과 동일 계열
...
from m08_apply_v2 import clean_text, load_docs, tier, HOLD_THRESHOLD, TRUST_THRESHOLD

TAX = PROC + "/business_taxonomy.parquet"
DETAIL = PROC + "/announcement_detail.parquet"
DOCS_E01 = REPORTS + "/e01_documents.jsonl"
...
def build_inputs(d, docs):
    ...
    out = {}
    # 현행: M08 이 원문 없는 건에 쓰던 구성 + 해시태그 (p02 의 text_for_model 과 동일 계열)
...
def main():
    ...
    print("학습 %d건 / %d클래스 (M08 과 동일 설정)" % (len(sub), len(classes)))
    ...
    save_report("m09_input_alignment.json", {
        "purpose": "학습/추론 입력 형식 정합이 판단보류율에 미치는 영향 분리 측정",
        "design": "모델·임계값·학습데이터를 M08 과 동일하게 고정하고 추론 입력 구성만 변경",
