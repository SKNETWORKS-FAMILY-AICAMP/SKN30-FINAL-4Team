...
공고량은 왜 지원성격으로 못 나누는가 (실측 근거)
    ...
    ㄷ. "원문을 더 받아오면 되지 않나"도 막힌다. 두 겹이다.
        - 2013~2018 은 사이트에 첨부 자체가 없다. 수집 당시 연도별 5건씩 탐침한
          결과 2013~2018 은 0/5, 2019~2025 는 5/5 였다(D02). 크롤링을 더 해도
          안 나온다. 원문 기반 라벨은 구조적으로 2019+ 로 제한된다.
        - 2019+ 는 원문이 있고 층화표본 5,000건(모집단 62,659 의 8%)을 이미
          받아뒀다. 가중치를 주면 구성비를 추정할 수 있을 것 같지만, 확신도
          임계값을 어디에 두느냐에 따라 구성비가 크게 흔들린다.
              사업화  21.9%(>=0.35) -> 30.7%(>=0.25) -> 39.4%(전량)
              융자    15.7%         -> 11.6%         ->  8.3%
          현행 기준 대비 TVD 0.11~0.13 으로, 제목 편향(0.146)과 같은 크기다.
          확신이 낮은 건이 최다 클래스인 사업화로 쏠리기 때문이다(M13 에서
          오답 5건 중 3건이 사업화였다). 판단보류된 52% 가 무작위가 아니다.

    그래서 공고량은 결측 0% 인 분야 축을 유지한다. 지원성격별 공고량 전망은
    분류기의 판단보류율이 크게 낮아지기 전에는 제공하지 않는다.

    (금액 축에는 이 문제가 같은 방식으로 걸리지 않는다. 모집단 구성비를 추정하는
     게 아니라 "라벨을 붙일 수 있었던 관측에서 유형별 금액 분포는 이렇다"는
     조건부 요약이고, 칸마다 관측수 n 을 함께 낸다. 다만 라벨 부여 여부가 금액과
     상관이 있다면 그만큼 치우칠 수 있어, 커버리지를 리포트에 남긴다.)
...
def main():
    ...
    save_report("a09_support_outlook.json", {
        ...
        "why_volume_not_by_support_type": {
            ...
            "why_more_crawling_does_not_help": {
                "2013_2018": ("사이트에 첨부 자체가 없다. 수집 당시 연도별 5건 탐침에서 "
                              "2013~2018 은 0/5, 2019~2025 는 5/5 였다(D02). "
                              "원문 기반 라벨은 구조적으로 2019+ 로 제한된다."),
                "2019_plus": ("층화표본 5,000건(모집단 62,659 의 8%)을 이미 받아뒀다. "
                              "그런데 확신도 임계값에 따라 구성비가 크게 흔들린다 — "
                              "사업화 21.9%(>=0.35)/30.7%(>=0.25)/39.4%(전량), "
                              "융자 15.7%/11.6%/8.3%. 현행 대비 TVD 0.11~0.13 으로 "
                              "제목 편향(0.146)과 같은 크기다."),
                "cause": ("확신이 낮은 건이 최다 클래스인 사업화로 쏠린다. M13 에서 "
                          "오답 5건 중 3건이 사업화였다. 판단보류 52% 가 무작위가 아니다."),
            },
            "conclusion": ("커버리지가 흔들려 건수를 세면 가짜 추세가 나고, 구성비도 "
                           "편향돼 총량 배분도 못 쓴다. 원문을 더 받아도 같은 크기의 "
                           "편향이 남는다. 분류기의 판단보류율이 크게 낮아지기 전에는 "
                           "제공하지 않는다."),
        },
