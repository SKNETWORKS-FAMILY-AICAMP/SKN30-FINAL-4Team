"""M15 — 커버리지를 올릴 때 정확도·오분류 편향이 어떻게 변하는지 측정.

왜 필요한가
    지원규모를 지원성격별로 보려는데 라벨 커버리지가 48~53% 라 연도 칸이
    한 자릿수다(A10). 커버리지를 올려야 추이를 볼 수 있다. 그런데 임계값만
    내리면 틀린 라벨이 섞여 들어와, 그 유형의 금액 분포 자체가 오염된다.

    특히 위험한 게 방향성 있는 오염이다. M13(수동 정답 41건)에서 오답 5건 중
...
검증 설계
    ...
    program_stem 으로 묶어 같은 사업이 학습·검증에 갈리지 않게 한다(M11 참고).
    도메인 밖 확인은 M13 수동 정답 41건으로 따로 한다.

주의
    학습셋 정확도는 도메인 내부값이라 실제 적용(2026 지자체 공고)보다 낙관적이다.
    M13 대조를 같이 보고, 최종 임계값은 두 곳에서 모두 견디는 값으로 고른다.
"""
import argparse
import json
import os
...
def amount_distortion(proba, y, le, thresholds, seed=42, n_boot=400):
    ...
    왜 시뮬레이션인가
        ...
        직접 비교할 표본이 안 된다. 그래서 두 재료를 합친다.
          ㄱ. 혼동행렬 — 임계값 t 에서 '유형 X 로 예측된 것' 중 실제 무엇이
              얼마나 섞이는지 (도메인 내부 OOF)
          ㄴ. 유형별 금액 분포 — 관측 테이블(A02)에서 유형별 per_company 금액
    ...
    # support_amount_observations 는 A02(timeseries-analysis) 산출물이라
...
def manual_check(thresholds):
    """도메인 밖 확인 — M13 수동 정답 41건. 예측 파일에서 확신도를 읽는다."""
...
def main():
    ...
    if mc:
        print()
        print("[도메인 밖] M13 수동 정답 대조 (Open API)")
    ...
    save_report("m15_coverage_accuracy.json", {
        ...
        "caveat": ("학습셋 정확도는 도메인 내부값이라 실제 적용(2026 지자체 공고)보다 "
                   "낙관적이다. M13 대조를 함께 보고 두 곳에서 모두 견디는 값을 고른다."),
        "output": OUT,
    })
    print()
    print("→ %s" % OUT)
