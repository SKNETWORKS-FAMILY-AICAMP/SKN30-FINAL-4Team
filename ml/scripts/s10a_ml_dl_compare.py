"""S10A — 모델 2·3·4 의 ML / DL 성능 통합 비교.

S07A~S09A(ML)와 S07B~S09B(DL)이 낸 측정값을 한 표로 모은다. 새로 학습하지 않고
...
def main():
    m2ml, m2dl = load("s07a_m2_cluster.json"), load("s07b_m2_cluster_dl.json")
    m3ml, m3dl = load("s08a_m3_cohort.json"), load("s08b_m3_cohort_dl.json")
    m4ml, m4dl = load("s09a_m4_anomaly.json"), load("s09b_m4_anomaly_dl.json")

    L = ["# 모델 2·3·4 — ML / DL 성능 비교", "",
         "> S07A~S09A(ML)와 S07B~S09B(DL)의 측정값을 모았습니다. 새로 학습하지 않고",
         "> 각 리포트 JSON 을 읽습니다.", "",
         "## 0. 한눈에", "",
    ...
    p = os.path.join(R, "s10a_ml_dl_compare.md")
    with open(p, "w", encoding="utf-8") as f:
        f.write("\n".join(L))
    print("\n".join(L))
    print("\n[report] %s" % p)

    C.save_report("s10a_ml_dl_compare.json", {
        "model2": {"ml": "s07a_m2_cluster.json", "dl": "s07b_m2_cluster_dl.json", "verdict": m2v},
        "model3": {"ml": "s08a_m3_cohort.json", "dl": "s08b_m3_cohort_dl.json", "verdict": m3v},
        "model4": {"ml": "s09a_m4_anomaly.json", "dl": "s09b_m4_anomaly_dl.json", "verdict": m4v},
    })
