...
ART_DIR = os.path.join(C.PROC, "m56_model2_canonical")
...
def main():
    ...
    try:
        with open(os.path.join(C.REPORTS, "m55_m2_leakage_audit.json"),
                  encoding="utf-8") as f:
            audit = json.load(f)
...
def write_md(payload, cmp_res, stab, iv_raw, iv_cqr, tcnt, tiers, rt, checks, verdict,
    ...
    p = os.path.join(C.REPORTS, "m56_m2_canonical.md")
