"""M08 — 지원성격 분류기 실전 적용 v2: 원문 품질·전처리 효과 분리 측정.

M07 대비 두 가지가 바뀌었다.
  ① 원문 추출본: E01(PyMuPDF + pyhwp) → E02(pdf-inspector + rhwp)
...
DOCS_E02 = REPORTS + "/e02_documents.jsonl"
OUT = PROC + "/announcement_detail_with_support_type_v2.parquet"

# 판단보류 임계값. M15 에서 커버리지·정확도·오분류 편향을 함께 재고 정했다.
#
#   M13 수동 정답 41건(실제 적용 도메인) 대조
...
def main():
    ...
    print("원문 보유 — E01 %d건 / E02 %d건" % (len(e01), len(e02)))
    print()
    ...
    save_report("m08_apply_v2.json", {
        ...
        "note": ("E02(pdf-inspector+rhwp) 원문과 전처리(관인제거+본문발췌)의 "
