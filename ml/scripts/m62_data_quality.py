...
def write_md(r):
    ...
    A("v1 은 상류(`business_taxonomy.parquet`)가 M32 파서 수정으로 갱신되기 **전에**")
    A("만들어져 그대로 얼어 있습니다. v2 를 지금 다시 만들면 그 갱신분이 같이")
    A("들어옵니다 — 근거문 수정과 섞어 세면 이번 수정의 크기를 부풀리게 되므로")
    A("따로 뺍니다.\n")
    A("| 컬럼 | 달라진 행 | 정체 |")
    A("|---|---:|---|")
    for k, v in r["ledger"]["상류_드리프트(이번_수정과_무관)"].items():
        A("| `%s` | %d | taxonomy 전용. M32 의 지원기업수 파싱 수정이 상류에 반영된 결과 |"
          % (k, v))
    A("")
    A("## 4. 바꾸지 않은 것\n")
    A("```text")
