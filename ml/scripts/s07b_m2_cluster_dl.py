"""S07B — 모델 2 딥러닝: Autoencoder latent embedding + 군집 + Ablation.

설계서의 모델 2 우선순위 4위가 "Autoencoder latent embedding + clustering" 이다.
1~3위(HDBSCAN / GMM / K-Means)는 S07A 에서 이미 쟀다.

기준선 (S07A, 지원성격 제외 조건 · 같은 1,447행)
    Gower + HDBSCAN   군집 20 / 노이즈 41.1% / 실루엣 0.6012
                      지원성격 ARI 0.2245 / bootstrap ARI 0.9545

공정한 비교를 위해 지킨 것
    AE 는 유클리드 잠재공간에서 군집하고 S07A 은 Gower 거리에서 군집한다.
    각자의 공간에서 실루엣을 재면 숫자가 서로 다른 자를 쓰는 셈이다.
    그래서 **군집 배정만 AE 에서 가져오고, 품질 점수는 S07A 과 똑같은 Gower
...
import common as C
from s07a_m2_cluster import (ARI_FAIL, CAT_FEATS, NUM_FEATS, SRC, gower,
...
# S07A 이 고른 HDBSCAN 설정과 같게 둔다. 군집기까지 바뀌면 무엇이 성능을 바꿨는지
...
def run(X, D, y_type, cfg):
    """AE 잠재공간에서 군집하고, 점수는 S07A 과 같은 Gower 거리에서 잰다."""
...
def main():
    ...
    # 점수용 자 — S07A 의 B 조건(지원성격 제외)과 정확히 같은 거리행렬
    D = gower(t, CAT_FEATS, NUM_FEATS)
    print("모델 2 DL 대상: %d행 / AE 입력 %d축" % (len(t), X.shape[1]))
    print("기준선(S07A Gower+HDBSCAN): 군집 20 / 노이즈 41.1% / 실루엣 0.6012 / ARI 0.2245")
    ...
    C.save_report("s07b_m2_cluster_dl.json", {
        "n_rows": int(len(t)), "n_input_axes": int(X.shape[1]),
        "hdbscan": HDB, "scoring": "군집은 AE 잠재공간, 점수는 S07A 과 같은 Gower 거리",
