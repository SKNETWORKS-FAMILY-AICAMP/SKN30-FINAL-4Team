"""M11 — 2022 병합이 준 성능 향상이 진짜인지 검증(누수 점검).
...
측정
    같은 데이터·같은 모델을 두 가지 분할로 나란히 잰다.
      ① StratifiedKFold       — M06 이 쓰는 방식. 같은 사업이 갈라질 수 있다.
...
def candidates(seed):
    """M06 상위 2개(선형 계열)만 본다. 트리 계열은 이 데이터에서 크게 뒤쳐진다."""
    return {
...
def main():
    ...
    save_report("m11_leakage_check.json", {
        ...
        "datasets": report,
        "caution": "일반 K-Fold 수치(M06 이 보고하는 값)는 두 해에 걸친 사업 때문에 "
