...
N02 가 이 모델에 Conditional 을 준 이유가 하나 있다.
...
def score_drivers(X, names, scores):
    ...
    아니라 '원문에 안 적힌 사업'을 탐지하고 있었다. N02 가 경고한 실패 모드다.
    """
...
def explain(row, ref):
    ...
    비교군 조회는 n04 의 `compare` 를 그대로 호출한다. 여기서 따로 구현하면
    같은 사업에 대해 모델 3 과 모델 4 가 서로 다른 비교군을 말하게 된다 —
    특히 지원단위 분리 같은 규칙이 한쪽에만 반영되는 사고가 난다.
    """
    from n04_m3_cohort import compare
...
def main():
    ...
    C.save_report("n05_m4_anomaly.json", {
...
def write_md(syn, best, cont, res, cases, verdict, train, df, drivers):
    ...
    for r in verdict["reasons"]:
        L.append("- %s" % r)
    L.append("")
    p = os.path.join(C.REPORTS, "n05_m4_anomaly.md")
