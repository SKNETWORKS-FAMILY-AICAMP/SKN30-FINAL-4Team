"""DL05 — KLUE-RoBERTa 로 Open API 재적용 (M08 의 딥러닝 버전).
...
M08 과의 비교가 성립하도록 조건을 동일하게 맞춘다.
    - 학습: 2023 엑셀 전량(900건, 26클래스)
    - 적용: Open API 1,570건, 원문 있으면 원문 / 없으면 요약문 대체
    - 전처리: M08 조건 B(관인부 제거 + 본문 마커 발췌) — M08 최적
    - 임계값: 0.25 / 0.35 (동일)
...
DOCS = DATA_DIR + "/e01_documents.jsonl"
...
# M08 (LogisticRegression) 결과 — 비교 기준
M08 = {"model": "TF-IDF + LogisticRegression", "condition": "B (E01 + 전처리)",
...
def main():
    ...
    print("설정: %s" % CFG)
    print("비교 대상 — M08(%s): 판단보류 %.1f%% (%d건)"
          % (M08["model"], M08["hold_rate"] * 100, M08["n_hold"]))
    print(flush=True)
    ...
    print("%-34s%10d%10d%10d%9.1f%%"
          % ("M08 (LogisticRegression)", M08["n_trust"], M08["n_ref"],
             M08["n_hold"], M08["hold_rate"] * 100))
    print("%-34s%10d%10d%10d%9.1f%%"
          % ("DL05 (KLUE-RoBERTa)", n_trust, n_ref, n_hold, n_hold / len(texts) * 100))
    print("=" * 74)
    print("평균 확신도: M08 %.4f → DL05 %.4f (%+.4f)"
          % (M08["mean_confidence"], conf.mean(), conf.mean() - M08["mean_confidence"]))
    print("사용가능:    M08 %d건 → DL05 %d건 (%+d)"
          % (M08["n_trust"] + M08["n_ref"], n_trust + n_ref,
             (n_trust + n_ref) - (M08["n_trust"] + M08["n_ref"])))
    ...
    with open(rp, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "hold_rate_change": round(n_hold / len(texts) - M08["hold_rate"], 4),
            "by_source": {
