"""DL09 — DL06 최적 조합으로 학습한 KLUE-RoBERTa 를 Open API 에 적용 + 정답 대조.
...
왜 별도 단계인가
    ...
    ②의 정답은 M13 의 수동 라벨 41건이다. 원문을 직접 읽고 붙였고, 적용 대상인
    2026 Open API 도메인이라 학습 도메인(2022~2023 중앙부처)과 겹치지 않는다.

M08(LogisticRegression) 과 조건을 맞춘다 — 안 그러면 비교가 성립 안 한다
    학습     business_taxonomy 전량, MIN_SUPPORT=10 -> 19클래스
    적용     announcement_detail 1,570건. 원문 있으면 원문(clean_text 전처리),
             없으면 요약문 대체 — M08 이 고른 조건 B 와 같다.
    임계값   0.20 / 0.35. M15 에서 커버리지·정확도·오분류 편향을 함께 재고
             정한 값이라 모델이 바뀌어도 같은 기준으로 본다.

무엇을 보나
    ㄱ. 판단보류율 — DL 이 확신도를 올려 커버리지를 늘리는가.
        이게 모델 2(A10)의 추세 검정력과 직결된다. M08 은 67.8% 사용가능이었다.
    ㄴ. M13 정답 41건 정확도 — 커버리지가 늘 때 정확도가 유지되는가.
        M08 은 판단보류 제외 29건 중 23건(79.3%) 이었다.
...
M08 = os.environ.get("DL_M08", "/workspace/dl/data/m08_pred.parquet")
...
# M15 에서 정한 값. 모델이 바뀌어도 같은 기준으로 봐야 비교가 된다.
HOLD_THRESHOLD = 0.20
...
def main():
    ...
    # ---- M08 과 나란히 ----
    cmp = {}
    if os.path.exists(M08):
        p8 = pd.read_parquet(M08)
        p8["announcement_id"] = p8["announcement_id"].astype(str)
        cmp = {"m08_usable_rate": round(float((p8["support_type_status"] != "판단보류").mean()), 4),
        ...
        print()
        print("[M08 대비] 사용가능률 %.1f%% -> %.1f%%"
              % (cmp["m08_usable_rate"] * 100, cmp["dl_usable_rate"] * 100))
        if "agreement" in cmp:
            print("  둘 다 확신한 %d건의 라벨 일치율 %.1f%%"
                  % (cmp["both_usable_n"], cmp["agreement"] * 100))

    os.makedirs(OUTDIR, exist_ok=True)
    with open(os.path.join(OUTDIR, "dl09_m1_apply_%s.json" % args.tag), "w",
              encoding="utf-8") as f:
        json.dump({
            ...
            "caveat": ("DL06 의 ablation 수치는 하이퍼파라미터 선택에 쓴 CV 라 선택 "
                       "편향이 있다. 편향 없는 평가는 여기의 M13 정답 대조다."),
            "output": outp,
        }, f, ensure_ascii=False, indent=2)
    print("\n[report] dl09_m1_apply_%s.json" % args.tag)
    print("→ %s" % outp)
