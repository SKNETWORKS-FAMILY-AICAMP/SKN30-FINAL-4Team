...
def write_md(r):
    ...
    if "extra" in r:
        ...
        A("잔차로 비교군 중앙값을 잡았고 그때는 0.4120 으로 기준(0.4133)보다 좋게")
        A("나왔습니다. 그런데 XGBoost 800 트리는 train 을 거의 맞히므로 그 잔차")
        A("중앙값은 0 근처로 눌리고, 보정이 아니라 **미세한 과적합 방향으로 밀어준**")
        A("것에 가깝습니다. train fold 안에서 inner GroupKFold(3) 로 OOF 잔차를 다시")
        A("잡자 0.4173 으로 뒤집혔습니다. **측정 방식이 결론을 만들던 자리**라")
        A("그대로 남깁니다.\n")
    ...
    A("**모델 2 에 준 것 전부**입니다.\n")
    A("```text")
    A("예측 오차          움직이지 않았다 (짝지은 차이의 95% 구간이 0을 품는다)")
    A("비교군 baseline    0.5315 -> 0.5277 로 내려갔다 (비교군이 더 동질해졌다)")
    A("예측구간           폭 25.1배 -> 24.4배, '제시 어려움' 등급 1칸 -> 0칸")
