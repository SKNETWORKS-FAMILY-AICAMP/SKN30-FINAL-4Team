"""S06G — S06F 최적 조합으로 학습한 KLUE-RoBERTa 를 Open API 에 적용 + 정답 대조.

왜 별도 단계인가
    S06F 의 ablation 수치(macroF1 0.8337)는 하이퍼파라미터를 고르는 데 쓴
    CV 성능이라 선택 편향이 있다. 그대로 일반화 성능이라 부르면 안 된다.
    편향 없는 평가는 두 가지다.
      ① 튜닝 전 base(0.8283) 와 ML 기준선(0.7953) 의 비교 — S06F 에서 이미 냈다
      ② 학습에 전혀 안 쓴 데이터로 재기 — 이 스크립트가 하는 일

    ②의 정답은 S05D 의 수동 라벨 41건이다. 원문을 직접 읽고 붙였고, 적용 대상인
    2026 Open API 도메인이라 학습 도메인(2022~2023 중앙부처)과 겹치지 않는다.

S05B(LogisticRegression) 과 조건을 맞춘다 — 안 그러면 비교가 성립 안 한다
    학습     business_taxonomy 전량, MIN_SUPPORT=10 -> 19클래스
    적용     announcement_detail 1,570건. 원문 있으면 원문(clean_text 전처리),
             없으면 요약문 대체 — S05B 이 고른 조건 B 와 같다.
    임계값   0.20 / 0.35. S05F 에서 커버리지·정확도·오분류 편향을 함께 재고
             정한 값이라 모델이 바뀌어도 같은 기준으로 본다.

무엇을 보나
    ㄱ. 판단보류율 — DL 이 확신도를 올려 커버리지를 늘리는가.
        이게 모델 2(S04G)의 추세 검정력과 직결된다. S05B 은 67.8% 사용가능이었다.
    ㄴ. S05D 정답 41건 정확도 — 커버리지가 늘 때 정확도가 유지되는가.
        S05B 은 판단보류 제외 29건 중 23건(79.3%) 이었다.
...
S05B = os.environ.get("DL_M08", "/workspace/dl/data/s05b_m1_apply_pred.parquet")
...
# S06F ablation 이 고른 조합
BEST = {"lr": 5e-5, "epochs": 8, "batch": 8, "max_len": 384,
        "class_weight": True}

# S05F 에서 정한 값. 모델이 바뀌어도 같은 기준으로 봐야 비교가 된다.
HOLD_THRESHOLD = 0.20
...
def main():
    ...
    if os.path.exists(LABELS):
        ...
        # S05D 라벨 파일에도 confidence(사람이 매긴 태깅 확신도) 컬럼이 있어,
    ...
    # ---- S05B 과 나란히 ----
    cmp = {}
    if os.path.exists(S05B):
        p8 = pd.read_parquet(S05B)
        p8["announcement_id"] = p8["announcement_id"].astype(str)
        cmp = {"s05b_m1_apply_usable_rate": round(float((p8["support_type_status"] != "판단보류").mean()), 4),
        ...
        print()
        print("[S05B 대비] 사용가능률 %.1f%% -> %.1f%%"
              % (cmp["s05b_m1_apply_usable_rate"] * 100, cmp["dl_usable_rate"] * 100))
        if "agreement" in cmp:
            print("  둘 다 확신한 %d건의 라벨 일치율 %.1f%%"
                  % (cmp["both_usable_n"], cmp["agreement"] * 100))

    os.makedirs(OUTDIR, exist_ok=True)
    with open(os.path.join(OUTDIR, "s06g_m1_dl_apply_v2_%s.json" % args.tag), "w",
              encoding="utf-8") as f:
        json.dump({
            ...
            "manual_eval": manual,
            "vs_m08": cmp,
            "caveat": ("S06F 의 ablation 수치는 하이퍼파라미터 선택에 쓴 CV 라 선택 "
                       "편향이 있다. 편향 없는 평가는 여기의 S05D 정답 대조다."),
            "output": outp,
        }, f, ensure_ascii=False, indent=2)
    print("\n[report] s06g_m1_dl_apply_v2_%s.json" % args.tag)
    print("→ %s" % outp)
