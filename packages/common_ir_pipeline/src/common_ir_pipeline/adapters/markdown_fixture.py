#!/usr/bin/env python3
"""Convert a manifest-declared Markdown fixture corpus into Common IR v1.
...
GENERATOR = "common_ir_pipeline.markdown_fixture"
...
def materialize_cleaned_input(
    ...
    # Keep this helper safe when called outside ``main`` as well.
    validate_input_manifest(original_input_dir, source_manifest)
...
def validate_input_manifest(input_dir: Path, manifest: dict[str, Any]) -> list[dict[str, Any]]:
    """Validate every declared semantic input before reading any of them.

    The fixture adapter deliberately accepts a non-empty corpus of any size,
    but every source must be a direct child of ``input_dir`` and must use the
    ``.input.md`` fixture suffix.  This prevents a manifest from materializing
    a Gold file, a sibling file, or a traversal target during presentation
    cleaning.
    """
    documents = manifest.get("documents")
    if not isinstance(documents, list) or not documents:
        raise ValueError("request manifest must contain a non-empty documents list")
    ...
    for index, item in enumerate(documents):
        ...
        if not isinstance(input_file, str) or not input_file:
            raise ValueError(f"manifest documents[{index}].input_file must be a non-empty string")
        if input_file in seen_files:
            raise ValueError(f"duplicate manifest input_file: {input_file}")
        if doc_id in seen_doc_ids:
            raise ValueError(f"duplicate manifest doc_id: {doc_id}")
        # A direct filename check rejects both traversal and nested paths
        # before resolving/opening the declared source.
        if Path(input_file).name != input_file or not input_file.endswith(".input.md"):
            raise ValueError(f"not an allowed semantic input filename: {input_file}")
        source_path = (resolved_root / input_file).resolve()
        if source_path.parent != resolved_root or not source_path.is_file():
            raise ValueError(f"semantic input must be a direct readable child of input_dir: {input_file}")
...
def main() -> int:
    ...
    # Validate the complete manifest before either cleaning or reading a
    # declared fixture source.
    documents = validate_input_manifest(original_input_dir, manifest)
    presentation_cleaning: dict[str, Any] | None = None
    ...
    if args.presentation_clean:
        ...
        manifest = json.loads(source_manifest_path.read_text(encoding="utf-8"))
        documents = validate_input_manifest(input_dir, manifest)
    else:
