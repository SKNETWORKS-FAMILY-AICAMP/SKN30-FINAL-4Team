...
from common_ir_pipeline.workers.pdf_surya_layout import (
    build_sidecar as build_surya_sidecar,
    layout_regions as surya_layout_regions,
)
from common_ir_pipeline.workers.promote_pdf_only_explicit_diagram_edges import main as promote_diagram_edges_main
...
class PortablePackageTests(unittest.TestCase):
    ...
    def test_surya_layout_sidecar_and_bridge_remain_textless(self) -> None:
        """Surya labels/geometry survive; its recognizer text never does."""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source_pdf = root / "notice.pdf"
            source_pdf.write_bytes(b"synthetic PDF input")
            secret = "Surya block OCR raw text must not become Common IR text"
            regions = surya_layout_regions({"bboxes": [{
                "label": "Table", "polygon": [[1, 2], [11, 2], [11, 9], [1, 9]],
                "recognition_text": secret,
            }]})
            sidecar = build_surya_sidecar(source_pdf, 1, 1.5, [{
                "page": 1, "image_size": {"width": 100, "height": 100},
                "render_seconds": 0.01, "layout_seconds": 0.02,
                "block_scan": {"attempted": 1, "records": [{"region_id": "surya-block-0", "recognition_seconds": 0.03, "completed": True}]},
                "total_seconds": 0.04, "regions": regions,
            }], "block")
            rendered = json.dumps(sidecar, ensure_ascii=False)
            self.assertNotIn(secret, rendered)
            self.assertFalse(sidecar["policy"]["surya_text_persisted"])
            self.assertEqual(sidecar["pages"][0]["regions"][0]["label"], "Table")
            sidecar_path = root / "surya-layout.json"
            ...
            output = root / "notice.common_ir_v1.json"
            with patch.object(sys, "argv", [
                "common-ir-pdf-native", "--notice-id", "TEST-SURYA", "--native", str(native_path),
                "--source-path", str(source_pdf), "--ocr-layout-diagnostic", str(sidecar_path), "--output", str(output),
            ]):
                pdf_native_main()
            document = json.loads(output.read_text(encoding="utf-8"))
            self.assertEqual(validation_errors(document), [])
            self.assertNotIn(secret, json.dumps(document, ensure_ascii=False))
            layout = next(block for block in document["blocks"] if block["block_id"].startswith("surya-layout:"))
            self.assertEqual(layout["text"], "")
            self.assertEqual(layout["source_block_label"], "Table")
            self.assertEqual(layout["occurrences"][0]["role"], "layout_region")

    def test_explicit_diagram_edge_requires_unique_native_endpoints_and_wires_to_common_ir(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source_pdf = root / "notice.pdf"
            source_pdf.write_bytes(b"synthetic PDF input")
            ...
            native_path.write_text(json.dumps({
                "method": "pdf_inspector", "version": "test", "process_result": {"page_count": 1},
                "text_items": [
                    {"page": 1, "text": "A", "x": 0, "y": 90, "width": 10, "height": 8, "font_size": 10},
                    {"page": 1, "text": "B", "x": 50, "y": 90, "width": 10, "height": 8, "font_size": 10},
                ],
            }), encoding="utf-8")
            ...
            evidence_path = root / "diagram-evidence.json"
            evidence_path.write_text(json.dumps({"pages": [{"page": 1, "high_accuracy_result": {"blocks": [{"label": "Diagram", "bbox": [0, 0, 60, 20], "html": "<table><tr><td><b>A</b></td><td>→</td><td><b>B</b></td></tr></table>"}]}}]}), encoding="utf-8")
            relations_path = root / "diagram-relations.json"
            with patch.object(sys, "argv", [
                "common-ir-pdf-promote-explicit-diagram-edges", "--source-pdf", str(source_pdf),
                "--native", str(native_path), "--render-manifest", str(render_path),
                "--surya-diagram-evidence", str(evidence_path), "--output", str(relations_path),
            ]):
                promote_diagram_edges_main()
            sidecar = json.loads(relations_path.read_text(encoding="utf-8"))
            self.assertEqual(len(sidecar["relations"]), 1)
            self.assertNotIn("<table", json.dumps(sidecar["blocks"], ensure_ascii=False))
            output = root / "notice.common_ir_v1.json"
            with patch.object(sys, "argv", [
                "common-ir-pdf-native", "--notice-id", "TEST-DIAGRAM", "--native", str(native_path),
                "--source-path", str(source_pdf), "--diagram-relations-pdf-base", str(relations_path), "--output", str(output),
            ]):
                pdf_native_main()
            document = json.loads(output.read_text(encoding="utf-8"))
            self.assertEqual(validation_errors(document), [])
            self.assertEqual(len(document["relations"]), 1)
            self.assertFalse(document["relations"][0]["inferred"])

    def test_explicit_diagram_edge_abstains_on_duplicate_native_label(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source_pdf = root / "notice.pdf"
            source_pdf.write_bytes(b"synthetic PDF input")
            ...
            native_path.write_text(json.dumps({"text_items": [
                {"page": 1, "text": "A", "x": 0, "y": 90, "width": 10, "height": 8},
                {"page": 1, "text": "A", "x": 20, "y": 90, "width": 10, "height": 8},
                {"page": 1, "text": "B", "x": 50, "y": 90, "width": 10, "height": 8},
            ]}), encoding="utf-8")
            render_path = root / "render.json"
            render_path.write_text(json.dumps({"render_scale": 1.0, "pages": [{"page": 1, "height": 100}]}), encoding="utf-8")
            evidence_path = root / "evidence.json"
            evidence_path.write_text(json.dumps({"pages": [{"page": 1, "high_accuracy_result": {"blocks": [{"label": "Diagram", "bbox": [0, 0, 60, 20], "html": "<table><tr><td>A</td><td>→</td><td>B</td></tr></table>"}]}}]}), encoding="utf-8")
            output = root / "relations.json"
            with patch.object(sys, "argv", ["edge", "--source-pdf", str(source_pdf), "--native", str(native_path), "--render-manifest", str(render_path), "--surya-diagram-evidence", str(evidence_path), "--output", str(output)]):
                promote_diagram_edges_main()
            self.assertEqual(json.loads(output.read_text(encoding="utf-8"))["relations"], [])
