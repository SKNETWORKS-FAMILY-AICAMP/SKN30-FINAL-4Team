#!/usr/bin/env bash
# Example: SURYA_INFERENCE_URL=http://127.0.0.1:8000 ./scripts/run_surya_layout.sh --pdf /abs/notice.pdf --output /abs/layout.json --pages 2,3
set -euo pipefail
...
exec "$PYTHON" -m common_ir_pipeline.workers.pdf_surya_layout --mode layout "$@"
