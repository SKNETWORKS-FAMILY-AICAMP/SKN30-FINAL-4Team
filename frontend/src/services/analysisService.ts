...
export const analysisService = {
    getCurrentAnalysis: async () => {
        try {
            return await api.get('/analysis/current')
        } catch (err: any) {
            if (err.status === 204 || err.status === 404) return null
            throw new Error(getFriendlyErrorMessage(err, '현재 분석 상태를 조회하는 중 오류가 발생했습니다'))
        }
    },
    ...
    closeSession: async (sessionId: string) => {
        try {
            return await api.post(`/analysis-sessions/${sessionId}/close`)
        } catch (error: any) {
            throw new Error(getFriendlyErrorMessage(error, '분석 세션을 종료하는 중 오류가 발생했습니다'))
        }
    },
}