  ...
  try {
    ...
    try {
      ...
      const accepted = await workerDispatcherFromEnvironment().dispatch({ schema_version: "analysis_worker_job/v1", analysis_run_id: runId, user_id: userId,
        source: { bucket: dispatch.source_bucket, object_key: dispatch.source_object_key, original_filename: run.original_filename ?? "source", content_type: run.declared_mime_type, size_bytes: run.declared_size_bytes ?? 0 },
        callback: { ingest_request_profile_url: callbackUrl, callback_token: callbackToken }, requested_at: new Date().toISOString() })
