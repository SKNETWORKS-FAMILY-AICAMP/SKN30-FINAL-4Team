...
2. migration 01~41을 적용해 private `existing-kb` bucket과 `kb`, `retrieval` schema를
   만들어야 한다.
...
완료 JSON의 `activation`은 정상 전환 시 `promoted_v2`다. `--limit`이나 `--strategy`
부분 실행은 절대 활성 전환하지 않는다.
