...
| 42 | `42_model1_runtime_manifest_refresh_v4.sql` | 29 | 0 | 0 | 통합 전 backend-rebuild의 중간 Model 1 v4 identity를 inactive 이력으로 보존; 현재 runtime은 v3 사용 |
...
| `workspace` | 27 | request 분석과 private analysis/chat queue state |
...
이 script에는 migration ledger가 없으며 매번 `01`~`42`를 모두 실행한다. 각 파일은 독립
...
배포 gate에는 별도로 migration 01~42 fresh apply, 기존 migration `01`~`32` 상태에서의
upgrade/replay, 현재 v3 Model 1 설정의 전체 분류·승격, 실제 private Storage put/get/delete,
