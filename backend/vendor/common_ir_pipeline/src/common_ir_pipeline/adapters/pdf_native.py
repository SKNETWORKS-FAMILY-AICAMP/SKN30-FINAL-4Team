...
def carry_ocr_layout_diagnostic_blocks(sidecar: dict, sidecar_path: Path, source_sha256: str) -> list[dict]:
    """Project a textless OCR layout sidecar into non-semantic Common IR blocks.
    ...
    is always empty.
    """
    if sidecar.get("sidecar_schema_version") != _OCR_LAYOUT_SIDECAR_SCHEMA:
        raise ValueError(f"unsupported OCR layout sidecar schema: {sidecar.get('sidecar_schema_version')!r}")
    ...
    if policy.get("ocr_text_persisted") is not False or policy.get("ocr_text_allowed_in_common_ir") is not False:
        raise ValueError("OCR layout sidecar policy permits OCR text")
    blocks = []
    ...
    for page_index, page in enumerate(sidecar.get("pages", [])):
        ...
        for region_index, region in enumerate(page.get("regions", [])):
            ...
            block_id = f"ocr-layout:p{page_number}:{region_id}"
            ...
            provenance = make_provenance(
                "cpu_ocr_text_region_geometry", page_number, [float(value) for value in bbox],
            ...
            blocks.append({
                ...
                "boundary_markers": [],
                "source_block_label": "ocr_text_region_geometry",
                "provenance": provenance,
