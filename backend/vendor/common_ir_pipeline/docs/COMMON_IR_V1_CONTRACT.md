...
이 패키지는 대응쌍의 위치를 요구하지 않는다. 각 HWP/HWPX/PDF source는
독립적으로 adapter에 전달하며, paired source는 lineage 또는 생성의 전제조건이
아니다.
...
패키지의 선택형 `common-ir-pdf-ocr-layout` worker는 CPU PaddleOCR/EasyOCR
recognition 결과를 사용해 region bbox·confidence·페이지 렌더링/엔진 시간을
관찰할 수 있다. 단 OCR 문자열은 sidecar에도 저장하지 않는다. 이 sidecar는
Common IR document도 CandidatePack 입력도 아니다. native PDF adapter가
`--ocr-layout-diagnostic`으로 source SHA-256을 확인한 뒤 blank
`layout_candidate`/`layout_region` provenance로 보존할 수는 있다. 이 경로는
table/cell/relation을 생성하지 않으며, CandidatePack 소비자는 빈
`layout_candidate`를 semantic 입력에서 제외해야 한다. 별도 명시 layout gate가
없는 한 table/cell/relation으로 승격되지 않는다.
...
과거 workspace의 회귀 산출물은 이 이식 패키지의 실행 전제조건이 아니다.
이식 후 검증은 package-local `tests/`와 각 배포 환경의 독립 fixture/source로
수행한다.
...
자동 회귀는 package-local `tests/`에서 실행한다.
...
이 문서는 사람이 읽는 안내이며, 기계적 최종 계약은 package-local
[`../src/common_ir_pipeline/schema.py`](../src/common_ir_pipeline/schema.py)다.
