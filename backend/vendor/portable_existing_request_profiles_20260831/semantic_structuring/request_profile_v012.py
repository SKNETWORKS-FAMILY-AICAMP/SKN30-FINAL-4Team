...
#   v5  a label-only table cell can govern only Common-IR cells that exactly
#       tile its explicit row/column geometry
VALUE_SPAN_CANDIDATE_GENERATOR_VERSION = "5"
...
def build_value_span_candidates(pack: CandidatePack) -> list[ValueSpanCandidate]:
    ...
    cache_basis = "\x1e".join((
        VALUE_SPAN_CANDIDATE_GENERATOR_VERSION,
        pack.generator or "",
        pack.generator_version or "",
        pack.pack_id,
        *(
            f"{block.block_id}\x1f{block.text}\x1f"
            + (
                ""
                if block._common_ir_cell_geometry is None
                else ",".join(str(value) for value in block._common_ir_cell_geometry)
            )
    ...
    for block in pack.blocks:
        ...
        for match in _PROGRAM_PERIOD_DATE_RANGE_FINDER.finditer(block.text):
            # ``_PROGRAM_PERIOD_DATE_RANGE_FINDER`` predates the field-region
            # rule and also recognises bare year ranges.  Those must be
            # supplied only by the region-bounded pass below: otherwise a
            # support-period or plan citation is silently offered as a
            # program_period candidate.  Month/day ranges remain document
            # wide as before.
            if _PROGRAM_PERIOD_YEAR_RANGE_FINDER.fullmatch(match.group()):
                continue
        ...
        for region in period_regions.get(block.block_id, ()):
            for match in _PROGRAM_PERIOD_YEAR_RANGE_FINDER.finditer(
                ...
                # The year grammar permits whitespace before its optional
                # ``년`` marker.  That can make the same visible span differ
                # from the older generic match only by its trailing space;
                # trim coordinates before deterministic de-duplication and
                # never make whitespace part of an anchor span.
                ...
                add_candidate(
                    block, start, end, "program_period_date_range"
                )
    ...
    *, model_id: str = "not_called", prompt_version: str = "request_source_selection_v0.1.4",
    ...
    for selected in selection.delivery_relations:
        ...
        if selected.relation_container.kind == "paragraph":
            ...
            actor_row = int(actor_cell.get("row_index") or 0)
            actor_row_span = max(1, int(actor_cell.get("row_span") or 1))
            # A merged actor cell occupies ``row_index`` through
            # ``row_index + row_span - 1``.  A non-empty cell starting inside
            # that interval (usually in another column) is not the next row
            # below the actor.  Keep this validation identical to
            # ``find_table_column_pairs`` so a server-generated candidate can
            # always pass the final fail-closed materializer.
            actor_end_row = actor_row + actor_row_span
            next_rows = [row for row in semantic_rows if row >= actor_end_row]
