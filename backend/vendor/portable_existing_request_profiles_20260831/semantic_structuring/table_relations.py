...
``row_span`` · ``col_index`` · ``col_span`` · ``text_occurrence_ids``). 블록 id 문자열을 파싱해
...
def find_table_column_pairs(
    ...
    for (row, col, span), actor in sorted(by_position.items()):
        # ``row_span`` 은 시작 행을 포함한 점유 행 수다. 예를 들어 r0에서
        # 2개 행을 합친 actor 셀은 r1을 덮으므로 member는 r2 이후에서
        # 찾아야 한다. 표의 다른 열에 r1 내용이 있어도 actor 아래의
        # 다음 의미 행은 아니다.
        actor_end_row = row + actor.row_span
        following = [item for item in semantic_rows if item >= actor_end_row]
        if not following:
            continue
