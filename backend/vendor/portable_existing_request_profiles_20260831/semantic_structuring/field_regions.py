...
FIELD_LABELS: dict[str, tuple[str, ...]] = {
    ...
    # 아래 넷은 요청서(서식 1)에 대응 행이 없는 필드다. 구역을 만들기 위해서가
    # 아니라 **라벨이 문서에 있는지 묻기 위해** 둔다. 라벨이 없으면 그 필드는
    # 양식에 칸이 없는 것이고, 라벨이 있는데 값이 비면 추출을 놓친 것이다.
    # 둘은 사용자가 할 다음 행동이 다르다.
    #
    # 표기는 관측한 것만 넣는다. Common IR 58 건에서 이 넷은 라벨 자리에 0 회,
    # 대조군 ``지원대상`` 199 회 · ``지원조건`` 115 회다. 관측 0 이라 변형
    # 표기는 추측하지 않고, 공고에서 실제로 확인된 ``실제 수혜자`` 만 함께 둔다.
    "applicant_eligibility": (r"신청\s*자격", r"지원\s*자격"),
    "beneficiary": (r"실제\s*수혜자", r"수혜\s*자"),
    "participation_requirements": (r"참여\s*요건", r"참가\s*요건"),
    "support_methods": (r"지원\s*방식", r"지급\s*방식"),
}
...
def _table_sibling_contents(
    blocks: list[SourceBlock], index: int
) -> list[tuple[SourceBlock, int, int]]:
    """Return only table contents whose ownership is explicit in geometry.

    A label merged across rows may own several cells immediately to its right.
    Those cells are safe only when their row intervals tile the label interval
    exactly, without a gap or overlap.  Returning every tiled cell lets the
    field-specific grammar pick its own values (for example, a year range)
    without this structural layer guessing that a header such as ``구분`` is
    the value.

    Geometry is trusted private metadata projected from Common IR.  A legacy
    or hand-authored table block without it fails closed instead of falling
    back to CandidatePack list adjacency.
    """

    label = blocks[index]
    label_geometry = label._common_ir_cell_geometry
    if (
        label.block_kind != "table_cell"
        or label.common_ir_block_id is None
        or label.common_ir_cell_id is None
        or label_geometry is None
    ):
        return []

    row, row_span, col, col_span = label_geometry
    ...
    by_cell: dict[
        str, tuple[tuple[int, int, int, int], list[SourceBlock]]
    ] = {}
    for candidate in blocks:
        if (
            candidate.common_ir_block_id != label.common_ir_block_id
            or candidate.common_ir_cell_id is None
            or candidate.common_ir_cell_id == label.common_ir_cell_id
        ):
            continue
        geometry = candidate._common_ir_cell_geometry
        # All cell blocks from one Common IR v1 table are projected together.
        # A missing coordinate means this pack cannot prove a unique tiling.
        if candidate.block_kind != "table_cell" or geometry is None:
            return []
        candidate_row, candidate_row_span, candidate_col, _ = geometry
        candidate_end = candidate_row + candidate_row_span
        if candidate_col != target_col or candidate_end <= row or candidate_row >= row_end:
            continue
        previous = by_cell.get(candidate.common_ir_cell_id)
        if previous is None:
            by_cell[candidate.common_ir_cell_id] = (geometry, [candidate])
        elif previous[0] != geometry:
            return []
        else:
            previous[1].append(candidate)

    tiled = sorted(
        by_cell.items(), key=lambda item: (item[1][0][0], item[0])
    )
    cursor = row
    for _, (geometry, cell_blocks) in tiled:
        cell_row, cell_row_span, _, _ = geometry
        cell_end = cell_row + cell_row_span
        if cell_row != cursor or cell_end > row_end:
            return []
        if not cell_blocks or any(
            starts_with_form_label(item.text or "") for item in cell_blocks
        ):
            return []
        cursor = cell_end
    right_hand = [
        item
        for _, (_, cell_blocks) in tiled
        for item in cell_blocks
        if (item.text or "").strip()
    ]
    if not right_hand or cursor != row_end or same_cell:
        return (
            [(item, 0, len(item.text or "")) for item in same_cell]
            if same_cell and not right_hand
            else []
        )
    return [(item, 0, len(item.text or "")) for item in right_hand]
...
def _sibling_contents(
    ...
    ``□ 사업추진 체계 및 절차`` 처럼 제목만 있는 일반 문단 뒤에 내용이
    따라오는 서식이 있다. 다음 라벨을 만나면 지배가 끝난다. 표 셀은 반드시
    Common IR 기하로만 연결하고 정렬 순서를 관계로 해석하지 않는다.
    """

    label = blocks[index]
    if label.block_kind == "table_cell" or label.common_ir_cell_id is not None:
        return _table_sibling_contents(blocks, index)
    for following in blocks[index + 1 :]:
...
def build_field_regions(
    ...
    for index, block in enumerate(blocks):
        ...
        for field in fields:
            ...
            if filled:
                located_spans = [(span, block) for span in filled]
            else:
                ...
                if not carry:
                    continue
                siblings = _sibling_contents(blocks, index)
                if not siblings:
                    continue
                located_spans = [
                    (
                    ...
                    for host, start, stop in siblings
                ]
            for span, host in located_spans:
...
# ``common_ir_v1.project_common_ir_v1`` derives table-cell CandidatePack ids as
# ``<table block id>#r<row>c<column>p<paragraph>``.  A few callers may also
# expose the intermediate cell coordinate without ``p<paragraph>``.  The table
# id itself is opaque and may describe a nested table (for example
# ``hwp:t87.c5.b1``), so only the final coordinate suffix has grammar here.
_TABLE_COORDINATE_SUFFIX = re.compile(
    r"^(?P<table_id>.+)#r(?P<row>\d+)c(?P<column>\d+)(?:p(?P<paragraph>\d+))?$"
)


def _is_table_ancestor(ancestor_id: str, descendant_id: str) -> bool:
    """Whether two CandidatePack ids name levels of one table coordinate.

    Raw string prefixes are not hierarchy: ``...p1`` and ``...p10`` are
    sibling paragraphs, just as ``...c1``/``...c10`` and ``t4``/``t40`` are
    siblings.  Only the two relationships defined by the projection grammar
    count: table parent -> cell/paragraph, and cell -> its paragraph.
    """

    if ancestor_id == descendant_id:
        return False
    match = _TABLE_COORDINATE_SUFFIX.fullmatch(descendant_id)
    if match is None:
        return False
    table_id = match.group("table_id")
    cell_id = f"{table_id}#r{match.group('row')}c{match.group('column')}"
    return ancestor_id == table_id or (
        match.group("paragraph") is not None and ancestor_id == cell_id
    )
...
def _drop_nested_duplicates(
    ...
    마찬가지다. 블록 id 가 실제 표 좌표 문법에서 부모·자식일 때만 좁은 쪽을
    남긴다. 문자열 접두사는 계층이 아니다. ``p1`` 과 ``p10`` 같은 형제를
    부모·자식으로 보면 실제 근거 하나가 조용히 사라진다.
    """
    ...
    for region in sorted(regions, key=lambda r: -len(r.block_id)):
        nested = any(
            ...
            and _is_table_ancestor(region.block_id, other.block_id)
            for other in kept
        )
        if not nested:
            kept.append(region)
