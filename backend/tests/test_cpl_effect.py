...
from worker.contracts.cpl_result import CplEvidence, CplFact, RECHECK_RECOVERED
...
def test_an_added_quote_is_promoted_with_its_own_span() -> None:
    ...
    assert (added.source_block_id, added.start_char, added.end_char) == (
        None,
        None,
        None,
    )
    assert added.evidence == [CplEvidence(None, "hwp:d1", "hwp:b3", ["occ:p3"])]
    # 기대효과만 재검한 문서는 목적 진단으로 오귀속되지 않는다.
    diagnostics = [
        row for row in result.diagnostics if row.stage == "cpl_purpose_recheck"
    ]
    assert [(row.unit, row.reason_code) for row in diagnostics] == [
        (EFFECT_FIELD, RECHECK_RECOVERED)
    ]
...
def test_returning_only_the_existing_quote_is_a_normal_empty_answer() -> None:
    ...
    # 추가도 실패도 아닌 no-op 이므로 복구라고 오기록하거나 빈 문자열을 쓰지
    # 않는다. StageDiagnostic 은 이 경우 명시적으로 reason 없음이다.
    diagnostics = [
        row for row in result.diagnostics if row.stage == "cpl_purpose_recheck"
    ]
    assert [(row.unit, row.reason_code) for row in diagnostics] == [
