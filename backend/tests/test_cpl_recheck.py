...
def test_a_recovered_fact_is_grounded_without_an_invented_id() -> None:
    ...
    # 재검은 Common IR occurrence로만 접지됐다. CandidatePack 좌표·블록을
    # 꾸며 내지 않고, Common IR 계보와 원문만 보존한다.
    assert (fact.source_block_id, fact.start_char, fact.end_char) == (None, None, None)
    assert [
        (
