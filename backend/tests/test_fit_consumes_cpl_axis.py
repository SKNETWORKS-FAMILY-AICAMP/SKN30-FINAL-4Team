...
from worker.contracts.fit_result import (
    PurposeAxisAssignment as FitPurposeAxisAssignment,
    PurposeAxisClassification as FitPurposeAxisClassification,
)
...
def test_fit_reexports_the_canonical_cpl_purpose_axis_contract() -> None:
    """FIT의 type annotation과 공개 import가 CPL 생산 타입과 동일하다."""

    assert FitPurposeAxisAssignment is CplPurposeAxisAssignment
    assert FitPurposeAxisClassification is CplPurposeAxisClassification
    assert FitResult.__annotations__["purpose_axis"] is CplPurposeAxisClassification
