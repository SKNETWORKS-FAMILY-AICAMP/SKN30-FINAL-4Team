...
from semantic_structuring.request_profile_v012 import (
    VALUE_SPAN_CANDIDATE_GENERATOR_VERSION,
...
def _table_pack(*cells: dict[str, object]) -> CandidatePack:
    occurrences: list[dict[str, str]] = []
    common_ir_cells: list[dict[str, object]] = []
    for index, cell in enumerate(cells):
        occurrence_id = f"occ:field-region:c{index}"
        text = str(cell["text"])
        occurrences.append({"occurrence_id": occurrence_id, "text": text})
        common_ir_cells.append(
            {
                "cell_id": f"field-region:c{index}",
                "row_index": cell["row"],
    ...
    document = {
        ...
        "document": {
            "document_id": "hwpx:field-region-table",
            "source_kind": "hwpx",
        },
        "blocks": [
            {
                "block_id": "hwpx:field-region-table:t0",
                "kind": "table",
                "structure_status": "explicit",
                "reading_order": 0,
                "text": " ".join(item["text"] for item in occurrences),
                "text_occurrence_ids": [
                    item["occurrence_id"] for item in occurrences
                ],
                "occurrences": occurrences,
                "cells": common_ir_cells,
            }
        ],
    }
...
def test_a_merged_table_label_exposes_every_exactly_tiled_right_cell() -> None:
    pack = _table_pack(
        {"row": 0, "row_span": 2, "col": 0, "text": "사업기간"},
        {"row": 0, "col": 1, "text": "구분"},
        {"row": 1, "col": 1, "text": "2024~2028년"},
    )

    regions = build_field_regions(pack, field_name="program_period")
    candidates = [
        item
        for item in build_value_span_candidates(pack)
        if item.candidate_kind == "program_period_date_range"
    ]

    # Geometry owns both cells; the period grammar, not adjacency or a text
    # guess, selects only the year-bearing one as a value candidate.
    assert [item.content_text for item in regions] == ["구분", "2024~2028년"]
...
@pytest.mark.parametrize(
    "right_cells",
    [
        # Gap: nothing covers the first row beside the merged label.
        ({"row": 1, "col": 1, "text": "2024~2028년"},),
        # Overlap: both cells claim the second row beside the label.
        (
            {"row": 0, "row_span": 2, "col": 1, "text": "구분"},
            {"row": 1, "col": 1, "text": "2024~2028년"},
        ),
    ],
)
def test_ambiguous_right_hand_table_geometry_fails_closed(
    right_cells: tuple[dict[str, object], ...],
) -> None:
    pack = _table_pack(
        {"row": 0, "row_span": 2, "col": 0, "text": "사업기간"},
        *right_cells,
    ...
    assert build_field_regions(pack, field_name="program_period") == []
    assert [
        item
        for item in build_value_span_candidates(pack)
        if item.value_raw == "2024~2028년"
    ] == []


def test_cell_geometry_is_private_and_separates_value_candidate_cache() -> None:
    merged = _table_pack(
        {"row": 0, "row_span": 2, "col": 0, "text": "사업기간"},
        {"row": 0, "col": 1, "text": "구분"},
        {"row": 1, "col": 1, "text": "2024~2028년"},
    )
    unmerged = _table_pack(
        {"row": 0, "row_span": 1, "col": 0, "text": "사업기간"},
        {"row": 0, "col": 1, "text": "구분"},
        {"row": 1, "col": 1, "text": "2024~2028년"},
    )
    label = next(
        block for block in merged.blocks if block.common_ir_cell_id == "field-region:c0"
    )

    assert label._common_ir_cell_geometry == (0, 2, 0, 1)
    assert "_common_ir_cell_geometry" not in label.model_dump(mode="json")
    assert "_common_ir_cell_geometry" not in SourceBlock.model_json_schema()[
        "properties"
    ]
    assert [
        item.value_raw for item in build_value_span_candidates(merged)
        if item.candidate_kind == "program_period_date_range"
    ] == ["2024~2028년"]
    # Same pack ID, block IDs and text, different geometry.  This must not hit
    # the merged pack's cached candidate set.
    assert [
        item.value_raw for item in build_value_span_candidates(unmerged)
        if item.candidate_kind == "program_period_date_range"
    ] == []
    assert VALUE_SPAN_CANDIDATE_GENERATOR_VERSION == "5"
...
# Nested Common IR table ids are opaque roots such as ``hwp:t87.c5.b1``.  The
# production CandidatePack appends its coordinate directly, without an extra
# path separator beyond ``#r``.
def test_nested_table_parent_and_production_paragraph_id_are_one_hierarchy() -> None:
    text = "○ (사업기간) 2026. 01. 01. ~ 2028. 12. 31."

    regions = _regions(
        ("hwp:t87.c5.b1", text),
        ("hwp:t87.c5.b1#r8c0p0", text),
    )

    assert [row.block_id for row in regions] == ["hwp:t87.c5.b1#r8c0p0"]


# CandidatePack paragraph indices are decimal tokens, not path-prefix text.
# With raw ``startswith``, the longer p10 id was kept first and the p1 sibling
# was then mistaken for its parent, silently dropping one real occurrence.
def test_table_paragraph_p1_and_p10_are_kept_as_siblings() -> None:
    text = "○ (사업기간) 2026. 01. 01. ~ 2028. 12. 31."

    regions = _regions(
        ("hwpx:t4#r3c1p1", text),
        ("hwpx:t4#r3c1p10", text),
    )

    assert {row.block_id for row in regions} == {
        "hwpx:t4#r3c1p1",
        "hwpx:t4#r3c1p10",
    }


# The same boundary rule applies to every decimal coordinate and to the opaque
# table id before ``#r``.  These pairs are siblings despite sharing text.
@pytest.mark.parametrize(
    ("left", "right"),
    [
        ("hwpx:t4#r3c1p0", "hwpx:t4#r3c10p0"),
        ("hwpx:t4#r1c1p0", "hwpx:t4#r10c1p0"),
        ("hwpx:t4#r3c1p0", "hwpx:t40#r3c1p0"),
    ],
)
def test_table_coordinate_prefixes_are_not_hierarchy(left: str, right: str) -> None:
    text = "○ (사업기간) 2026. 01. 01. ~ 2028. 12. 31."

    regions = _regions((left, text), (right, text))

    assert {row.block_id for row in regions} == {left, right}
