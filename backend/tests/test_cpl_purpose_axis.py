...
# 한 원문이 축 둘을 가지면 원본 fact 와 축마다의 투영 fact 를 모두 남긴다.
# 축이 단수라야 소비 쪽 필터가 (필드, 축) 한 쌍으로 끝난다.
def test_one_statement_keeps_original_and_two_axis_facts() -> None:
    llm = _Llm([
    ...
    # 축 fact 의 값은 실제 인용 구간이고, 구조화 1차 값도 보존한다.
    assert [fact.value_raw for fact in facts] == [
        _VALUE, "부산 관내 제조 중소기업", "기술경쟁력을 강화"
    ]
    # 구조화 1차 fact 는 그대로이고, 축 투영분만 새 id 없이 추가한다.
    assert [fact.fact_id for fact in facts] == ["fact_1", None, None]
    assert [fact.evidence_ref for fact in facts[1:]] == [_ref(), _ref()]
    # 축 인용문은 Common IR occurrence로만 접지된다. CandidatePack block/span을
    # 같은 이름으로 위장하지 않는다.
    assert [(fact.source_block_id, fact.start_char, fact.end_char) for fact in facts[1:]] == [
        (None, None, None), (None, None, None)
    ]
    assert [
        (e.source_block_id, e.common_ir_document_id, e.common_ir_block_id,
         e.common_ir_occurrence_ids)
        for fact in facts[1:]
        for e in fact.evidence
    ] == [
        (None, "hwpx:d1", "hwpx:t4", ["occ:p0"]),
        (None, "hwpx:d1", "hwpx:t4", ["occ:p0"]),
    ]
...
def test_each_axis_keeps_its_own_evidence_row() -> None:
    ...
    assert len(_purpose_facts(with_axes)) == 3
    axis_rows = rows(with_axes)
    assert [row["raw_value"] for row in axis_rows] == [
        _VALUE, "부산 관내 제조 중소기업", "기술경쟁력을 강화"
    ]
    promoted_rows = axis_rows[1:]
    assert [row["candidate_pack_block_id"] for row in promoted_rows] == [None, None]
    assert [row["common_ir_block_id"] for row in promoted_rows] == ["hwpx:t4", "hwpx:t4"]
    # 같은 축이 같은 구간을 두 번 주면 그건 여전히 한 줄이다.
    twice = _run(_profile(), _Llm([
        _row(CplAxisCode.DIRECTION.value, "기술경쟁력을 강화"),
        _row(CplAxisCode.DIRECTION.value, "기술경쟁력을 강화"),
    ]))
    assert len(rows(twice)) == 2
...
def test_a_partial_axis_response_keeps_original_fact_and_evidence() -> None:
    llm = _Capture([{
        "evidence_ref": _partial_ref(),
        "axis_code": CplAxisCode.TARGET_CONDITION.value,
        "quoted_text": _HEAD,
    }, {
        # 유효한 한 축만 승격해도, 누락하거나 잘못 준 다른 축 때문에 원본
        # value/evidence 가 지워지면 안 된다.
        "evidence_ref": _partial_ref(),
        "axis_code": "NOT_A_PURPOSE_AXIS",
        "quoted_text": "기술경쟁력",
    }])
    ...
    # 1 차 값은 꼬리 절이었지만, 유효 축은 구역 앞절에서 별도 fact 로 승격된다.
    assert [(f.axis_code, f.value_raw) for f in facts] == [
        (None, _TAIL),
        (CplAxisCode.TARGET_CONDITION.value, _HEAD)
    ]
    original = facts[0]
    assert original.fact_id == "fact_1"
    assert [
        (e.source_block_id, e.common_ir_document_id, e.common_ir_block_id,
         e.common_ir_occurrence_ids)
        for e in original.evidence
    ] == [("hwpx:t4", "hwpx:d9", "hwpx:t4", ["occ:p0"])]
    assert result.purpose_axis.dropped == [_partial_ref()]
    # 축 fact는 실제 인용문을 value로 쓰되 Common IR occurrence로만 접지한다.
    # 잘린 1차 값의 CandidatePack 좌표를 물려받거나 Common IR block을 Pack으로
    # 재명명하지 않는다.
    promoted = facts[1]
    assert (promoted.source_block_id, promoted.start_char, promoted.end_char) == (
        None, None, None
    )
    assert [
        (e.source_block_id, e.common_ir_document_id, e.common_ir_block_id,
         e.common_ir_occurrence_ids)
        for e in promoted.evidence
    ] == [(None, "hwpx:d9", "hwpx:t4", ["occ:p0"])]
    assert promoted.evidence_ref == _partial_ref()
...
def test_an_ambiguous_quote_in_one_purpose_region_is_never_promoted() -> None:
    """축 응답에 occurrence 내 위치가 없으므로 중복 문구를 임의로 고르지 않는다."""

    region = "○ (사업목적) 사업성장 지원으로 사업성장 기반을 마련"
    ir = {
        "document": {"document_id": "hwpx:duplicate"},
        "blocks": [{
            "block_id": "hwpx:t-duplicate",
            "occurrences": [{"occurrence_id": "occ:duplicate", "text": region}],
        }],
    }
    from worker.cpl_coverage import build_fragments
    ...
    profile = {
        "comparison_profile": {"purpose_goal": [{
            "fact_id": "fact:duplicate", "value_raw": "사업성장 지원",
            "status": "identified",
            "evidence": [{
                "source_block_id": "hwpx:t-duplicate",
                "common_ir_document_id": "hwpx:duplicate",
                "common_ir_block_id": "hwpx:t-duplicate",
                "common_ir_occurrence_ids": ["occ:duplicate"],
            }],
        }]},
        "field_states": [{"field_name": "purpose_goal", "status": "identified"}],
    }
    ...
    result = analyze_cpl(
        profile,
        _Llm([_row(CplAxisCode.DIRECTION.value, "사업성장", ref)]),
        model_profile="default",
        common_ir=ir,
    )

    assert result.purpose_axis.assignments == []
    assert result.purpose_axis.dropped == [ref]
    assert [fact.axis_code for fact in _purpose_facts(result)] == [None]
