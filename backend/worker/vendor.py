...
- 2026-09-12 ``field_regions.FIELD_LABELS`` 에 네 필드를 더했다.
  ...
  · ``support_methods``. 요청서에 해당 라벨 행이 있는지
  ``absent_form_fields`` 가 물을 수 있게 하는 문법이다. 이 넷은 요청서(서식 1)에
  대응 행이 없어 구조화 결과만 보면 ``not_found`` 이다. 서버의
  ``_with_form_absence`` 가 라벨 부재와 빈 값을 함께 확인한 경우에만
  ``not_applicable`` 로 올려 CPL-10·11 항목 전체를 ``needs_confirmation`` 으로
  끌어내리지 않게 한다. 판별기준 §11.3 · §12.3 은 "조건이 별도로
  없다고 해서 자동으로 오류로 판단하지 않는다" 고 못 박는다.
