...
#
# 동작하는 큐는 ``backend/worker/jobs.py`` 다. claim(SKIP LOCKED)·heartbeat·
# fencing 을 180 줄로 구현하고 실제 두 커넥션 경합으로 검증했다.
#
# 지우지 않고 남기는 이유: 워커 시도를 ``ops.processing_run`` 행으로 남기는
# 발상은 초안 §9.5 의 실행 진단(실패 단위·호출 ID·보완 이력)에 그대로 쓸 수
# 있다. jobs.py 는 지금 attempt_count 만 센다. 그 확장을 붙일 때 참고한다.
#
