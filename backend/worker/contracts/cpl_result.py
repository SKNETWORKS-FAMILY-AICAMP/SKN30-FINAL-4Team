...
@dataclass(frozen=True, slots=True)
class CplEvidence:
    """Fact의 CandidatePack 또는 Common IR 근거 한 줄.

    ``source_block_id`` is a CandidatePack block identifier.  A recheck that
    was grounded only to a Common IR occurrence must leave it ``None`` rather
    than relabelling the Common IR block as a CandidatePack block.
    """
...
class CplFact:
    ...
    ``source_block_id`` 및 문자 좌표는 CandidatePack 좌표가 검증된 경우에만
    채운다. Common IR occurrence만 알고 있는 경우의 계보는 ``evidence``의
    ``common_ir_*`` 필드에 남기며, 그 block/offset을 이 필드에 재사용하지 않는다.
    """
