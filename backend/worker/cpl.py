...
def _representative(subfields: list[CplSubfield]) -> tuple[str, str | None]:
    ...
    빈 목록은 현재 매핑에서는 나오지 않지만, 향후 대응 필드가 없는
    항목이 생겨도 ``no_content`` 로 단정하지 않는 방어 규칙이다. 문서를
    그 항목으로 읽을 필드가 없었다면 내용이 비었다는 근거가 아니므로
    ``needs_confirmation`` / ``NO_PROFILE_FIELD`` 를 돌려준다.
    """
...
def _classify(
    ...
    *,
    ...
    원문에 **한 번만** 나오는지 검사하고 통과하지 못한 행은 버린다. 같은 문구가
    둘이면 모델이 어느 자리를 뜻했는지 알 수 없으므로 첫 번째를 임의 선택하지
    않는다. 버린 사실은 ``dropped`` 에 남긴다. 예산은 1 이며 같은 입력으로
    재시도하지 않는다.
    """
    ...
    for row in response.assignments:
        ...
        if (
            ...
            continue
        first = fragment.raw_text.find(row.quoted_text)
        if fragment.raw_text.find(row.quoted_text, first + 1) >= 0:
            # Axis response에는 occurrence 내 위치를 표현할 필드가 없다. 중복
            # 인용문을 첫 번째에 붙이면 모델의 선택을 조용히 바꾸는 셈이다.
            dropped.append(row.evidence_ref)
            continue
        assignments.append(
            PurposeAxisAssignment(
...
def _with_axes(
    ...
    def expand(subfield: CplSubfield) -> CplSubfield:
        if subfield.profile_field != _PURPOSE_FIELD:
            return subfield
        # 축은 원본 fact 의 투영이지 대체물이 아니다. 같은 occurrence 에서 축을
        # 하나라도 얻었다고 1 차 value/evidence 를 빼면, 부분 응답이 나왔을 때
        # 원래 값과 감사 근거가 사라진다. 원본을 모두 보존한 뒤 축 fact 만 더한다.
        return replace(subfield, facts=[*subfield.facts, *promoted])
...
def _axis_fact(fragment: CplFragment, row: PurposeAxisAssignment) -> CplFact:
    """검증한 인용문을 Common IR occurrence 근거와 함께 fact 로 올린다."""
    ...
    return CplFact(
        ...
        status="identified",
        # Axis classification is grounded to a Common IR occurrence, not a
        # CandidatePack span.  Do not relabel its block or occurrence-relative
        # offsets as CandidatePack coordinates.
...
class DeliveryRelationDraft:
    ...
    아직 Fact 가 아니다. Common IR occurrence와 member 종류까지 확정했을
    뿐이고, 관계를 CPL 결과에 올리는 것은 별도 단계다.
    """
    ...
    for decision in decisions:
        ...
        drafts.append(
            DeliveryRelationDraft(
                ...
                common_ir_document_id=candidate.common_ir_document_id,
    ...
    for row in rows:
        ...
        facts.append(
            CplFact(
                ...
                status="identified",
                # Recheck candidates are Common IR occurrences, not
                # CandidatePack spans.  Their relative location is verified
                # above, but must not be persisted as CandidatePack offsets.
                source_block_id=None,
                start_char=None,
                end_char=None,
...
def _delivery_fact(draft, occurrence, *, member, index, relation_id):
    return CplFact(
        ...
        status="identified",
        # Table-cell occurrence selection proves Common IR grounding only.
        # The full table is not a CandidatePack block and occurrence-local
        # offsets are not available, so do not fabricate either claim.
        source_block_id=None,
        start_char=None,
        end_char=None,
        ...
        evidence=[
            CplEvidence(
                source_block_id=None,
                common_ir_document_id=draft.common_ir_document_id,
    ...
    for row in rows:
        ...
        facts.append(
            CplFact(
                ...
                value_raw=quote,
                status="identified",
                # As with purpose/delivery recheck values, this is verified
                # against a Common IR occurrence, not a CandidatePack span.
                source_block_id=None,
                start_char=None,
                end_char=None,
...
def _with_recheck(result, common_ir, llm_client, *, model_profile, candidate_pack=None):
    ...
    # 재검은 한 번의 호출이지만 목적·수행관계·기대효과는 독립된 판정 단위다.
    # 호출 전체를 목적에만 기록하면 effect/delivery 만 재검한 경우의 이력이
    # 엉뚱한 필드에 붙고, 정상적인 기대효과 빈 응답은 빈 reason_code 로 남는다.
    # 활성 섹션마다 결과를 남긴다. 기대효과에서 추가할 것이 없었던 정상 응답은
    # 복구도 실패도 아니므로 reason_code 를 None 으로 둔다. 빈 문자열로 실패를
    # 위장하지 않고, 복구했거나 실패했을 때에만 해당 코드를 기록한다.
    diagnostics = [*result.diagnostics]
    for unit, recovered, failure_reason in (
        (_PURPOSE_FIELD, bool(facts), outcome.purpose_reason),
        (_DELIVERY_FIELD, bool(delivery_facts), outcome.delivery_reason),
        (EFFECT_FIELD, bool(outcome.effect_facts), outcome.effect_reason),
    ):
        active = (
            (unit == _PURPOSE_FIELD and bool(fragments))
            or (unit == _DELIVERY_FIELD and bool(candidates))
            or (unit == EFFECT_FIELD and bool(effect_regions))
        )
        if not active:
            continue
        diagnostics.append(
            StageDiagnostic(
                ...
                unit=unit,
                reason_code=RECHECK_RECOVERED if recovered else failure_reason,
                message=detail,
                attempt=1,
            )
        )
