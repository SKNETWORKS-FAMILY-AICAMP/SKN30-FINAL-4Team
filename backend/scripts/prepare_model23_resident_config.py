#!/usr/bin/env python3
"""Prepare protected token and pinned identity for resident Model 2/3.

The command never prints either value.  By default it is idempotent only when
both existing outputs are private regular files and still match the verified
``ml`` serving closure.  ``--refresh-identity`` may atomically replace only a
valid stale identity; it never replaces the existing token.
"""
...
from prereview_model23_service.runtime import verified_runtime_manifest_sha256


DEFAULT_ML_ROOT = REPOSITORY_ROOT / "ml"
DEFAULT_TOKEN_FILE = (
    REPOSITORY_ROOT / ".runtime" / "model23-serving" / "model23-service.token"
)
DEFAULT_ENV_FILE = (
    REPOSITORY_ROOT / ".runtime" / "model23-serving" / "model23-resident.env"
)
MANIFEST_ENV = "PREREVIEW_MODEL23_REMOTE_RUNTIME_MANIFEST_SHA256"
    ...
    *,
    ...
    try:
        ...
        raise ResidentConfigurationError(
            "configuration output is not a private file"
        ) from None
    if (
...
def _ml_owner_for_configuration(ml_root: Path) -> tuple[int, int] | None:
    """Return the POSIX owner shared by the ML tree and Compose users."""

    if os.name != "posix":
        return None
    metadata = ml_root.stat()
    ml_owner = (metadata.st_uid, metadata.st_gid)
    caller_owner = (os.geteuid(), os.getegid())
    if 0 in ml_owner or 0 in caller_owner or ml_owner != caller_owner:
        raise ResidentConfigurationError(
            "Model 2/3 ML root owner must match the non-root effective caller UID:GID"
        )
    return ml_owner
...
def _replace_identity_atomically(
    ...
    *,
    ...
    if _read_existing_manifest(path, expected_owner=expected_owner) != expected_manifest:
    ...
    try:
        ...
        with os.fdopen(descriptor, "w", encoding="ascii", newline="\n") as stream:
            ...
            stream.flush()
            os.fsync(stream.fileno())
        _private_regular_file(temporary, expected_owner=expected_owner)
...
def prepare_configuration(
    *,
    ...
    token_file: Path,
    env_file: Path,
    manifest_resolver: Callable[[Path], str] = verified_runtime_manifest_sha256,
    ...
    expanded_ml_root = ml_root.expanduser()
    if expanded_ml_root.is_symlink():
        raise ResidentConfigurationError("Model 2/3 ML root must not be a symlink")
    ml_root = expanded_ml_root.resolve(strict=True)
    if not ml_root.is_dir():
        raise ResidentConfigurationError("Model 2/3 ML root must be a directory")
    expected_owner = _ml_owner_for_configuration(ml_root)
    ...
    if refresh_identity and (token_exists or env_exists):
        if not (token_exists and env_exists):
            raise ResidentConfigurationError(
                "resident configuration is only partially present"
            )
    ...
    try:
        manifest = manifest_resolver(ml_root)
    except Exception:
        raise ResidentConfigurationError(
            "resident runtime identity verification failed"
        ) from None
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if created and args.refresh_identity and complete_pair_existed:
        ...
        action = "검증 후 재사용"
    print(f"Model 2/3 상주 서비스 설정을 {action}했습니다. 비밀값은 출력하지 않았습니다.")
